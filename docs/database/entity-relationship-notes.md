...
Refina o ER de `04_...` §2 com `audit_events`, `artist_metric_id` em `report_items` e o veredito Data/AI de manter `report_runs` unificado no MVP (OD-DB-01 fechado sem split).
...
|---|---|---|
| **RAW** | Insert-only. Sem `UPDATE`/`DELETE`. Recoleta = novo `run_id`. **Trigger de imutabilidade obrigatório (SEC-D03).** | `raw_youtube_search_pages`, `raw_youtube_videos`, `raw_youtube_channels` |
| **COMPUTED** | Reconstruível por `run_id` + `rubric_hash`/`rule_version`. Recompute é determinístico; linhas referenciadas por snapshot publicado não são apagadas. Nunca edição manual de número. | `video_artist_mappings`, `channel_eligibility`, `artist_metrics` |
| **SNAPSHOT** | Congelado após `reports.status = published`. Valores copiados, não derivados em leitura. Leitura do produtor via VIEW pública (SEC-F03). | `reports` (pós-publish), `report_items` |
| **EVENT** | Append-only. Estado é agregação de eventos; correção = evento compensatório. **`audit_events` com trigger obrigatório; `producer_events` recomendado (SEC-D03).** | `producer_events`, `audit_events`, `artist_aliases`, `wtp_responses` |
| **STATE** | Só colunas de status mutáveis, via máquina de estados; transição sensível → `audit_events`. | `producers`, `applications`, `admin_users`, `followups`, `report_runs`, `artists` |

> **SEC-F01 (estrutural):** `service_role` faz **bypass de RLS**. Por isso a imutabilidade de RAW e `audit_events` é garantida por **trigger no banco** (SEC-D03), não só por grants/RLS, e a authz nos caminhos service-role vive **no código do handler**.
...
Derivações determinísticas do raw: mapeamento vídeo→artista, elegibilidade de canal, métricas e Score (componentes 40/25/20/15, velocity mediana, signals, diversidade de canais), `raw_score`/`final_score`. Sempre carregando `rubric_version` + `rubric_hash` + `computed_from_video_ids` + `metrics_detail_json`.

### O que entra em REPORT SNAPSHOT
Os valores **congelados** materializados no publish: `score_display`, `score_value`, `signals`, `velocity_display`, `competition_level`, `competition_channel_count`, `example_video_id`/`example_url`, `tag` (HOT) e a prova (`selection_reason_json`). **Nem tudo é público (SEC-F03):** o produtor lê via VIEW só os campos públicos (`score_display` etc.); `score_value`, `raw_score`, `selection_reason_json` cru e `metrics_detail_json` ficam admin/server-only — RLS é por linha, não por coluna.
...
report_items.score_value / signals / velocity_display / competition_channel_count
  └─ report_items.artist_metric_id  ──►  artist_metrics
        ...
        └─ metrics_detail_json            ──►  auditoria por célula (vídeos aceitos/rejeitados, canais, componentes)
```
...
report_items.competition_level / competition_channel_count
  └─ artist_metrics.channel_diversity_count
        ├─ computed_from_video_ids[] ──► raw_youtube_videos (run_id, video_id, channel_id)
...
Critério (`04_...` §14, `03_...` §12): um relatório é reconstruível a partir de `run_id` + `rubric_version`/`rubric_hash`.

```txt
run_id + rubric_hash ─► raw_youtube_* (imutável)
        ...
        ─► [rebuild] report_items determinísticos
              apontando report_items.artist_metric_id para a linha exata de artist_metrics
        ─► comparar com o report_items congelado  →  devem ser idênticos nas células computadas
```

Se diferir sob o mesmo `run_id` + `rubric_hash`, é bug metodológico (não "ajuste no olho"). Data/AI ratifica que o MVP não precisa de split: re-score sob novo `rubric_hash` usa o mesmo `run_id` e grava/recompõe `artist_metrics` com chave lógica `(run_id, artist_id, rubric_hash)`.
...
- **Computed:** `(run_id, artist_id, rubric_hash)` único em `artist_metrics`; `(run_id, channel_id)` único em `channel_eligibility`; `(run_id, video_id)` único em `video_artist_mappings` como resolução canônica.
...
- **Proibir update em RAW/`audit_events`:** **trigger `BEFORE UPDATE/DELETE` obrigatório** (SEC-D03) além de grants/RLS — `service_role` faz bypass de RLS (SEC-F01). Detalhe em `rls-review-notes.md` §3.
...
- ❌ expor `report_items.*` cru ao produtor (RLS por linha) → vaza `score_value`/`raw_score` escondido; usar VIEW pública (SEC-F03).
- ❌ derivar admin de `user_metadata` (editável pelo usuário) → usar `admin_users` + `is_admin()` (SEC-D02).
- ❌ confiar a imutabilidade do raw só a grants/RLS → service-role bypassa; exige trigger (SEC-D03/SEC-F01).
