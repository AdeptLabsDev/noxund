...
**Tabelas:** `producers`, `applications`, **`admin_users`** (SEC-D02 — incluída já na Fase 1 porque a RLS de quase tudo depende de `is_admin()`).
**Por quê primeiro:** sem dependências; habilita o approval gate e `/apply`.
**Inclui:**
- `producers.auth_user_id uuid unique null` (SEC-D01); **sem** coluna de senha/hash (SEC-F12).
- `admin_users` + helper `is_admin()` `SECURITY DEFINER` (SEC-D02); admin nunca derivado de `user_metadata`.
- enums de status; unicidade `lower(email)`; índice parcial de aplicação aberta.
- **`/apply` whitelist (SEC-F02 — bloqueante):** `anon` com **zero grant direto**; escrita só por handler service-role com whitelist estrita de campos do aplicante; `status` forçado a `pending`/`submitted` no servidor; rate limit. (Contrato de handler é do Backend; o schema apenas não deve facilitar mass-assignment.)
**Critério (backlog [DB] Schema base):** aplicação pode ser enviada e aprovada; FKs e enums corretos; aplicante não consegue setar `status`/`reviewed_by`/`approved_at`/`auth_user_id`.
**Revisão:** Database + Security (PII, identidade SEC-D01, role admin SEC-D02, `/apply` SEC-F02).
...
## Fase 8 — Audit Events
**Tabelas:** `audit_events` (OD-DB-03 aprovado em DEC-0003, condicionado à RLS do Security).
**Inclui:** append-only; **trigger `BEFORE UPDATE/DELETE` obrigatório** (SEC-D03); polimórfico `(entity_table, entity_id)`; `before_json`/`after_json`; `actor_id` nulo para ações de pipeline.
**Critério:** ações sensíveis (aprovação, publicação, overrides, grant/revoke de admin) registradas e imutáveis.
**Revisão:** Security (acesso restrito admin/service-role + trigger SEC-D03), Database.
...
|---|---|
| 1 | OD-02 (Auth: Supabase Auth) → `producers.auth_user_id`; SEC-F02 (`/apply` whitelist, desenho Backend); SEC-D02 (`admin_users` + `is_admin()`) |
...
| 8 | OD-DB-03 (aprovado, condicionado à RLS do Security) |
