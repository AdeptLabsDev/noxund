...
**Por quê antes de computed:** `artist_metrics`/`report_runs` (Fase 5) referenciam `rubric_version` + `rubric_hash`.
**DDL autorado (não aplicado):** `supabase/migrations/20260620000002_phase2_versioning.sql` · rollback em `supabase/rollback/20260620000002_phase2_versioning.rollback.sql`.
**Inclui:**
- `version` único + **`unique (version, hash)`** em ambas → alvo da **FK futura por `(rubric_version, rubric_hash)`** na Fase 5.
- `hash` determinístico **computado pelo data-engine** (Data/AI), **não fabricado no banco**; `config_json` opaco (sem semântica de rubric no DB — evita drift).
- **Imutabilidade** (versão publicada nunca é editada — nova versão = nova linha): trigger `BEFORE UPDATE/DELETE` (row) + `BEFORE TRUNCATE` (statement) nas duas tabelas, função `versioning_row_immutable()` (princípio SEC-0003 §2). Backbone da reprodutibilidade.
- **RLS** `ENABLE` + default-deny; **zero `GRANT`/policy a `anon`/`authenticated`** (SEC-F13/F02); `config_json`/`hash` internos (produtor vê só `X/100` + tooltip, §7).
- **Seed do rubric §7** como template comentado (40/25/20/15 verbatim); `rubric_hash` inserido coordenado com Data/AI. **Score/rubric inalterado.**
**Critério:** `version` único; `hash` determinístico; FK-ready; Score/rubric inalterado.
**Revisão:** **Database** (autor) + **Security** (matrix #3 — RLS/trigger no SQL concreto antes do apply) + **Data/AI** (matrix #5 — fidelidade ao §7; owner do `rubric_hash`).
**Gate de `run_migration`:** espelha a Fase 1 — humano + required reviewers; Security re-revisa o SQL.
...
**DDL autorado (não aplicado):** `supabase/migrations/20260620000003_phase3_runs_artists.sql` · rollback `supabase/rollback/20260620000003_phase3_runs_artists.rollback.sql` · verify `supabase/tests/phase3_post_apply_verify.sql` (paridade §4/§5 com Fases 1–2).
**Inclui:**
- 2 enums (`report_run_status`, `artist_alias_source`).
- **`report_runs` = âncora de proveniência:** STATE (status/contadores), mas **identidade de coleta (keyword/vertical/janela) congelada** e **DELETE/TRUNCATE bloqueados** por trigger (recoleta = novo `run_id`) — serve "raw imutável preservada". `rubric_version/rubric_hash` nullable (rubric do relatório publicado); rubric por-scoring vive em `artist_metrics` (Fase 5, DATA-AI-0001) — **não** congelado aqui.
- **Dedupe de artista:** `unique lower(canonical_name)` em `artists`; `unique lower(alias)` global em `artist_aliases` (+ FK `artist_id` `ON DELETE CASCADE`). Sem trigger de imutabilidade (merge humano precisa de mutabilidade) — correções via `audit_events`.
- **RLS** `ENABLE` + default-deny nas 3; **zero `GRANT`/policy a `anon`/`authenticated`** (SEC-F13/F02); revoke explícito. As 3 são internas (produtor vê `report_items`, Fase 5).
**Nota:** `reports`/`report_items` só na Fase 5 (dependem de computed).
**Revisão:** **Database** (autor) + **Security** (matrix #3 — RLS/triggers no SQL) + **Data/AI** (matrix #5 — identidade/dedupe de artista; confirmar placement de `rubric_*` em `report_runs` vs `artist_metrics`).
**Gate de `run_migration`:** espelha Fases 1–2 (humano + required reviewers; verify §4/§5 fail-closed).
