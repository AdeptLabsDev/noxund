...
**Additive B5 status note — verified against canonical main `fae24ba7551a5d9469e7b1d62aa175a1ebf7a90c`, 2026-08-20.** The bullets above are B4's record at its own `3a1c9865…` base and are **not rewritten**. Their current status, so that a pack assembled after B5 does not restate an overtaken one: **(i) and (iii) repaired** — B5 reconciled `current-state.md` §B / §G / §H / §I and retired the `context-map.md` `†` note, so the strings quoted in (i), in (iii) and in the §5 case-A trap are **historical as of B4's base**, not current text; the failure *class* each illustrates is unchanged and still worth learning from. **(ii) unchanged** — the freshness condition is inherent, applies to this document too, and now runs from `fae24ba…`. **(iv) repaired** — `current-state.md` §G records B4 as LANDED. On the two classification questions in the last bullet: the `/context` blanket-versus-table ambiguity has **no residual routing consequence** — every `/context` file already carries a landed `DEC-0035` §9 classification — while the class of `scope-guardrails.md` stands at **`HOLD — PRODUCT-LEAD ADJUDICATION REQUIRED`**, escalated and unresolved (`context-map.md` §1). **A pack cites the status above; it still closes neither question.**

**Additive `DEC-0036` status note — verified for this note only against canonical main `262c71907f24375196d13dc6901f64aa354acfad`, 2026-08-20.** Neither the B4 bullets nor the B5 note above is rewritten; both are accurate records at their own bases. **One of the two classification questions is no longer open, so the instruction to cite its open status is itself now overtaken.** The class of `scope-guardrails.md` was adjudicated by the Product Lead and landed by [`DEC-0036`](decisions/DEC-0036-scope-guardrails-authority-class-descriptive-current.md): **`DESCRIPTIVE-CURRENT · CURRENT · EDITABLE`, no INTERNAL-NORMATIVE authority of its own**. A pack assembled from here on **cites that adjudicated class, not a HOLD**, and routes any scope constraint to its **governing source** rather than to the compilation — the pointer at `docs/agents/product-orchestrator-agent.md`:167 incorporates nothing (`DEC-0036` D2–D4). The `/context` blanket-versus-table question is **unchanged**: still not adjudicated, still with no residual routing consequence. **A pack closes neither question; it now reports one as decided.**
