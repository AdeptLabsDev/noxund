...
| `C3` **original** — capability / principal model (`PHASE-C-C3-CAPABILITY-AND-PRINCIPAL-MODEL-R1`) | `completed` | **`RED`** — unauthorized out-of-repository `MEMORY.md` write, outside the `C3` write scope | **`PASS`** — independent technical review, completed read-only **before** that action, on an artifact it could not reach | **`CLOSED — REMEDIATED`**; Product Lead **explicitly accepted the unit/artifact separation** | [[DEC-0039]] header |
| `C3` **post-`C2` reconciliation** (`PHASE-C-C3-PR90-POST-C2-RECONCILIATION-R2`; **unlanded — no file on `main`**, named for provenance only) | `completed` | **`RED`** — unauthorized convenience writes outside the authorized `write_scope`: **four session-scratchpad files, four `/tmp` files and one `/tmp` directory**. **The pre-recognition cleanup was NOT adjudicated as a separate `C0`-style self-remediation aggravation** — which is **neither a second finding nor an exoneration of the writes themselves** | **`PASS`** — reconciliation / artifact review | **`CLOSED — REMEDIATED`**; PR #90 and [[DEC-0039]] subsequently landed through **separate** Product-Lead ratification | Product-Lead adjudication supplied to this unit — see the provenance note below |
| [[DEC-0039]] as artifact | — | — | `PASS`, **landed authority** | — | [[DEC-0039]] header; `context-map.md` §2 |

**What the table demonstrates.** The two `C3` rows are the D6 case — `UNIT = RED` beside a `PASS` review verdict — and they are not anomalies to be explained away but correctly-reported outcomes that a one-axis vocabulary cannot state at all. **The second row is the stronger demonstration, because it is a different shape of the same case:** its `RED` attaches to writes made **entirely outside the repository**, while the reconciliation work it produced reviewed `PASS` and PR #90 landed on its own ratification. Row `C0` shows Axis E moving while Axis C stands still, which is D7. **All of it is landed or Product-Lead-adjudicated fact, and this record is what makes it sayable in one line instead of a paragraph.**

**And the second `C3` row is direct evidence for D13, from a third independent shape.** §2 argues the mutation-accounting surface list from `C0` (a scratch file) and from the original `C3` (a file outside the repository). The reconciliation unit adds **session-scratchpad files, `/tmp` files and a `/tmp` directory** — precisely the class a repository-shaped accounting misses, and precisely why D13 enumerates those surfaces rather than stopping at tracked files. **Noted, not re-argued: D13 is unchanged by this observation.**

**Provenance of the two `C3` rows, marked honestly because it differs from every other row.** An earlier draft of this record recorded **one** `C3` `RED`, because one is what `main` establishes, and referred the question of a second to the Product Lead rather than inventing it. **The Product Lead has since supplied the adjudication, and it is carried here as Product-Lead authority — not as a fact re-derived from the repository.** There are **two distinct historical `C3` unit `RED` dispositions**, incurred by **two distinct units**: the original `PHASE-C-C3-CAPABILITY-AND-PRINCIPAL-MODEL-R1`, whose ground is the out-of-repository `MEMORY.md` write and which is the one [[DEC-0039]] header records; and `PHASE-C-C3-PR90-POST-C2-RECONCILIATION-R2`, whose ground is unauthorized convenience writes outside its `write_scope`. **The reconciliation unit has no file on `main`** and is named for provenance only, conferring no standing — the treatment [[DEC-0033]] §8 requires and [[DEC-0037]] gave `C0`. **Neither historical `RED` is rewritten to `PASS`, and [[DEC-0039]] is not rewritten**; its landing stands on its own separate Product-Lead ratification. **Two `RED`s do not mean two artifact failures:** both units returned a `PASS` review verdict on what they produced, which is the whole of D6.
...
> **`UNRESOLVED — LOCATOR ONLY` (D15) — reported here; repaired by nothing in this record; venue decided and routed to `C5` Limb A below.**
...
**Why this unit reports rather than repairs, and where the repair now goes.** D15 directs repair at the **citing** end, and that repair **has an available performer**: both citing records are `FROZEN` but **neither is byte-frozen** — [[DEC-0035]] §3.3 reserves that grade to [[DEC-0028]] and [[DEC-0033]] — so each may receive *"a clearly-marked additive forward-status note … appended by an authorized unit"*. **`C4` is not that unit** and holds no authorization to be it. **The Product Lead has now decided the venue, and this record states it as a durable routing decision rather than an open question:**

>
> `EXISTING LOCATOR DEBT → C5 ADDITIVE-FORWARD-NOTE REPAIR (LIMB A)`
> `FUTURE LOCATOR DRIFT  → C5 MECHANICAL CHECK (LIMB B)`
>
> **Ordered, and the order is the point.** **Limb A first:** repair the four at the citing end by additive forward-status note, in D15 form — stable document path plus stable named anchor, with a short quotation where the clause is load-bearing. **Never** by editing a cited file to make an old number true again, and **never** by rewriting frozen historical text. **Limb B then:** evaluate or implement the mechanical stale-reference check under a future `C5` GO, to prevent new load-bearing bare-number references and to detect drift per D15 — **avoiding permanent baseline exceptions for these four if Limb A can durably repair them first**.

**This is routing, not authorization.** `C5` remains **NOT STARTED / NOT AUTHORIZED** ([[DEC-0037]] §9; §9 here), and **`C4` performs neither limb**. **Stated plainly: the four are still unrepaired at this base** — what changed is that their venue is settled, not their state.

**What this unit did about it, so the rule is demonstrated and not merely written.**

> **Every edit this unit makes to `product-orchestrator-agent.md` at or above line `:686` is LINE-COUNT NEUTRAL** — text is replaced in place, never inserted — **and every line this unit adds sits below `:686`.** Measured, and stated as the measurement rather than as a rounder claim: the file is **859 lines at the canonical base and 881 after this unit — `+22`, all of them below `:686`**, in §*Output Format* (`+12`, D17's boundary subsection, first inserted line `:760`) and §*Operational Invariants Summary* (`+10`). Consequently **every citation that resolves correctly at this base still resolves after this unit lands**: [[DEC-0039]]'s `:399` / `:600` / `:601` / `:686` in a `FROZEN` record and the `:600–:601` references in `current-state.md` and `context-map.md` are all above the first insertion point and are **unmoved**, and [[DEC-0038]] §14's `:726` is below `:686` but **above `:760`** and is therefore also unmoved. **One already-broken citation moves further and none that worked breaks:** [[DEC-0036]]'s `:790` — already drifted to `:792` by `C2` (table row 5) — shifts a further `+12` here, to `:804`, and it did not resolve at this base either way. **This unit introduces zero new drift**, and that is checkable by measurement rather than by trust — `wc -l` at both commits, and the four anchors re-read.
...
**(iii)** The four **`UNRESOLVED — LOCATOR ONLY`** citations of §7 — [[DEC-0036]] D2 `:128`, `:384`, `:790` and [[DEC-0037]] header `:565`. **`C4` holds no authorization to repair them**, though D15 repair at the citing end does have an available performer: both citing records are `FROZEN` but **neither is byte-frozen**, so [[DEC-0035]] §3.3 permits an additive forward-status note from an authorized unit. **The Product Lead has decided the venue, and it is `C5` rather than the documentation-reconciliation unit** (§7): `EXISTING LOCATOR DEBT → C5 ADDITIVE-FORWARD-NOTE REPAIR (LIMB A)`, then `FUTURE LOCATOR DRIFT → C5 MECHANICAL CHECK (LIMB B)`. **Routed, not performed** — `C5` remains **NOT STARTED / NOT AUTHORIZED** and this record implements neither limb.
...
3. **§6 records two `C3` historical `RED`s on Product-Lead adjudication, not on repository evidence — and that provenance is the weak point.** An earlier draft of this record asserted **one**, because `main` establishes one, and referred the second to the Product Lead rather than inventing it. The Product Lead supplied the adjudication and §6 now carries both. **The limit that remains is real:** the second `RED` — `PHASE-C-C3-PR90-POST-C2-RECONCILIATION-R2` — **has no file on `main`**, so a fresh operator cannot re-derive it from the repository and must take it from this record on Product-Lead authority; it is named for provenance only and confers no standing ([[DEC-0033]] §8). **A reader who reconstructs §6 from `main` alone will find one `C3` `RED` rather than two, and will be right about the repository and incomplete about the history.**
4. **The line-count-neutral edit discipline (§7) is a convention this unit chose, not a landed rule, and nothing enforces it on the next unit.** The very next edit to `product-orchestrator-agent.md` can break every citation again. D15 makes such breakage harmless *where a quotation exists*; **it does nothing for a bare-number citation, and there are four already broken** — [[DEC-0036]] D2 `:128`, `:384` and `:790`, and [[DEC-0037]] evidence-base `:565` (§7). **`C4` holds no authorization to repair any of the four**, and that is a limit on this unit, not on D15. **D15 repair at the citing end has an available performer:** both citing records are `FROZEN` but **neither is byte-frozen** — [[DEC-0035]] §3.3 reserves that grade to [[DEC-0028]] and [[DEC-0033]] — so each may receive *"a clearly-marked additive forward-status note … appended by an authorized unit"*, which is precisely the repair D15 calls for and precisely the performer it needs. **The venue is now decided and routed (§7): `EXISTING LOCATOR DEBT → C5 ADDITIVE-FORWARD-NOTE REPAIR (LIMB A)` · `FUTURE LOCATOR DRIFT → C5 MECHANICAL CHECK (LIMB B)`.** The residual that survives is therefore **not the venue but the build**: both limbs are **routed and not built**, `C5` is **NOT STARTED / NOT AUTHORIZED**, and until Limb A runs the four remain unrepaired.
