...
`product-orchestrator-agent.md` §*Output Format* carries a short boundary note recording this and nothing more (§11).
...
| [[DEC-0036]] §Scope, §2, D2 | `:167` | `:169` | `:167` now resolves to the `WTP` scope bullet |
| `task-context-pack.md` §5 | `:167` | `:169` | same drift; **EDITABLE, outside this unit's write scope — routed, not repaired** (§11) |
...
**The classification, applied rather than asserted.** Under D15 every drifted row above is a **stale locator, not an authority defect**. [[DEC-0037]], [[DEC-0035]] and [[DEC-0036]] are all **`FROZEN` and are not edited**, and none is weakened: each quotes the clause it relies on, so the quoted text and the named clause govern and the reference resolves. **The one exposed case is [[DEC-0037]]'s `:565`**, which is cited *with* its quotation in D5's body but appears as a bare number in the evidence-base header — reachable by the quotation, and precisely the reason D15 requires both.

**What this unit did about it, so the rule is demonstrated and not merely written.**

> **Every edit this unit makes to `product-orchestrator-agent.md` at or above line `:686` is LINE-COUNT NEUTRAL** — text is replaced in place, never inserted — **and lines are added only below `:686`.** The file is **859 lines before this unit and 859 lines after it**. Consequently **every citation that resolves correctly at this base still resolves after this unit lands**, including [[DEC-0039]]'s `:399` / `:600` / `:601` / `:686` in a `FROZEN` record and the `:600–:601` references in `current-state.md` and `context-map.md`. **This unit introduces zero new drift**, and that is checkable by measurement rather than by trust.
...
**1. `docs/agents/product-orchestrator-agent.md` — six exact points.** Mutability established first: `docs/agents/**` agent contracts are INTERNAL-NORMATIVE within their declared agent-behaviour scope and **EDITABLE** ([[DEC-0035]] §9).
...
| §*Operational Invariants Summary* | the breach line becomes `ANY VERIFIED AUTHORIZATION BREACH IN A GOVERNED UNIT => UNIT RED`, with the D1 non-collapse invariant and the D13 accounting invariant added |
...
**Deliberately left alone, and named rather than swept.** `docs/agents/orchestration-runtime.md`; `README.md`; `agent-registry.md`; `agent-boundaries.md`; `agent-review-matrix.md`; `global-agent-rules.md`; `agent-onboarding-orchestration.md`; the ten agent contracts and `orchestration-runtime-engineering-agent.md`; and the corpus-wide historical-normalization residual (`PHASE-B-CLOSEOUT-R1` §5 row 1). **Two residuals this unit newly identifies are routed to the same Product-Lead-authorized reconciliation unit [[DEC-0038]] §14 already established, and are not swept into `C4`:** (i) `docs/product/task-context-pack.md` §5's `:167` citation, drifted by the same +2 (§7); and (ii) the remaining `AgentResult` mentions in `product-orchestrator-agent.md` §*Definition of Done* and §*Stage-Gate Discipline*, which are readable as the `ROLE RESULT` once the §*Trust model* heading names the mapping, and which this unit deliberately does not chase.
...
4. **The line-count-neutral edit discipline (§7) is a convention this unit chose, not a landed rule, and nothing enforces it on the next unit.** The very next edit to `product-orchestrator-agent.md` can break every citation again. D15 makes such breakage harmless *where a quotation exists*; it does nothing for a bare-number citation, and [[DEC-0037]]'s evidence-base header contains one. **The durable fix is the `C5` staleness check (§9 item 5), which is routed and not built.**
