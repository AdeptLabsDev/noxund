...
**Governance provenance — unit conduct and artifact verdict, deliberately kept separate:** `C3 UNIT = HISTORICAL RED` · `C3 GOVERNANCE FINDING = CLOSED — REMEDIATED` · `ARTIFACT INDEPENDENT TECHNICAL REVIEW = PASS` · `PRODUCT LEAD = EXPLICITLY ACCEPTED THE UNIT / ARTIFACT SEPARATION`. The `RED` attaches to the **unit's conduct** — an unauthorized persistent write to an out-of-repository `MEMORY.md` file, outside the `C3` write scope, adjudicated by an independent governance reviewer — and **not to this record**, whose independent technical review was completed **read-only, before that action, on an artifact the action could not reach**. That separation is expressly permitted by `product-orchestrator-agent.md`:600 — *"artefato tecnicamente aceito pode continuar com um finding de governança histórico, desde que o gate atual permita e o Product Lead aceite explicitamente essa separação"* — and the historical `RED` is **preserved and never rewritten to `PASS`** (`:601`). **Both line numbers re-derived at the post-`C2` reconciliation base.** **`DEC-0039` is therefore eligible for ratification and is not itself `RED`.**
...
**Negative scope — this record does not create or change:** any GitHub principal, App, bot, service account, token, credential or secret · any ruleset, branch protection, `CODEOWNERS` file, reviewer requirement, Environment, variable or repository setting · any workflow, workflow registration, CI check, validator, lint or hook · any agent registration, activation, contract, boundary, runtime handler, runtime allow-list or runtime wiring · `packages/orchestrator/**` in any respect, **including its disposition** · any preserved ref, branch, stash or archive · any database, SQL, migration, role or runner · any cloud resource · any collection arming state · any code file · any product scope, MVP admission, `OD-*` item or Axis-1 technical question · the `AgentResult` schema · any **mechanical** enforcement of anything stated here · memory content — **`DEC-0039` and the repository patch that lands it create or change no memory content.** The `C3` **unit** separately incurred a historical governance breach through an unauthorized out-of-repository `MEMORY.md` edit; that action is **outside this record's authority and effect**, **contributes no evidence to this record**, and is preserved in the unit's governance provenance (see *Governance provenance* above). **No principal is created, no agent is wired, no agent is registered, and no execution of any kind is authorized.**
...
**Discharges — by satisfaction, not by replacement:** the `APPROVER PROVENANCE = OPEN · ROUTED TO C3` assignment [[DEC-0038]] §13 and §15 made. **The routed item was a question assigned for adjudication, and it is discharged by being adjudicated — not by being solved**, which is the distinction [[DEC-0038]] §3 draws about its own discharge of [[DEC-0032]] §8. §7 and D6 return the answer: **`TECHNICAL APPROVER-PROVENANCE = NOT CURRENTLY SEPARABLE FROM SINGLE PRINCIPAL`**, with **P3 — HYBRID** adopted at D9. **No authenticated provenance is implemented and no trust anchor is provisioned.** `DISCHARGES` is the exact token because the deferred *decision* is now made while the *technical* problem stays open — `EXTENDS` would misdescribe an answered assignment as a supplied limb, and no supersession token applies, [[DEC-0038]] being neither edited nor narrowed. [[DEC-0038]] stays **CURRENT** and **FROZEN**, and its **REDUCE** disposition is neither rewritten nor reopened by this record.
...
**Does not edit:** any prior DEC. [[DEC-0001]] … [[DEC-0038]] all remain **frozen and additive-only**; [[DEC-0028]] and [[DEC-0033]] remain **byte-frozen** per [[DEC-0034]] §4. No prior record is declared obsolete, superseded, reopened or reinterpreted. **`DEC-0038` is the highest landed decision record on `main`** — re-derived at the post-`C2` reconciliation base (§3, E21); at `C3`'s initial evidence base it was [[DEC-0037]], `C2`'s landing is what changed it, and **this record's own landing advances it to `DEC-0039`** (E21). **`DEC-0038` is therefore landed *before* `DEC-0039`, and `DEC-0039` remains the Product-Lead-reserved identifier for this unit.** The `0038` gap that existed while this record was drafted was a consequence of **authorized parallel execution**, not of missing authority, and it asserted nothing about any other unit or its work. **This record does not rewrite, supersede, reopen or weaken [[DEC-0038]]'s `REDUCE` disposition**; it answers only the approver-provenance question [[DEC-0038]] §13 routed here. **No agent contract, boundary, registry entry or review matrix is edited — this unit touches no file under `docs/agents/**`.**
**Evidence base:** every configuration and repository claim below was re-derived in this unit at the **initial `C3` evidence base**, is marked `VERIFIED` in §3, and carries the command or file that produced it. Each was then **re-issued read-only at the post-`C2` reconciliation base**; the four that moved are repaired in their rows and marked `RE-DERIVED @ 1e7e124a`. Claims that could only be established by performing a forbidden action are marked `UNKNOWN — NOT PROBED` and are **not** resolved by assumption in either direction. Corpus surfaces read and not edited: `docs/agents/README.md`, `agent-registry.md`, `agent-boundaries.md`, `agent-review-matrix.md`, `product-orchestrator-agent.md`, `global-agent-rules.md`, `agent-conflict-resolution.md`, `orchestration-runtime.md`, `agent-onboarding-orchestration.md` §4 and §9; [[DEC-0035]] §3.1, §3.2, §4 P3/P4, §6, §7, §9, §10, §11, §12; [[DEC-0036]] §4; [[DEC-0037]] D1–D13, §6, §9, §10; `current-state.md` §D, §E; `context-map.md` §1–§3; `PHASE-B-CLOSEOUT-R1` §5.
**Two bases, distinguished rather than collapsed** ([[DEC-0035]] §4 P4). **INITIAL `C3` EVIDENCE BASE** = `main` @ `45667387f74f92cf9aca532f786f9c8bd519d788` — where every §3 measurement E1–E21 was taken. **POST-`C2` RECONCILIATION BASE** = `main` @ `1e7e124a2374917c95bffbb56f5f515d95132ee2`, the descendant merge landing [[DEC-0038]] (PR #89, 2026-08-21). **This record does not pretend its evidence was gathered at the later base.** Every §3 fact that could have moved was **re-derived** at the reconciliation base and is marked `RE-DERIVED @ 1e7e124a` in its row; facts left unmarked were measured at the initial base and are unchanged there.
...
Re-derived in this unit at the **initial `C3` evidence base**, and re-issued at the **post-`C2` reconciliation base** (see the re-derivation paragraph below). Configuration values are a **dated observation**, not a rule: per the construction [[DEC-0037]] D12 used of itself, **the normative content of this record does not depend on any particular setting** — §4 forbids overstating separation rather than relying on a value. Volatile state is owned by `current-state.md`, which carries the freshness contract; this snapshot exists so §4's conclusions are anchored rather than asserted.

**No secret, token, key or credential value was read, and none is recorded here.** Only presence, absence and name.

**Post-`C2` re-derivation, so the two bases are not conflated.** E1–E21 were measured at the **initial `C3` evidence base**. At the **post-`C2` reconciliation base** `1e7e124a…` each was re-issued read-only and the results compared. **Unchanged and re-confirmed:** E1, E2, E3, E5, E6, E7, E8, E9, E10, E11, E12, E13 — 11 workflow files on `main`, all still declaring `permissions: contents: read`, against 12 registry rows including the same orphan `322938835` — E16, E18, E19, E20. **Moved, and repaired in the rows below:** **E14** (PR #89 has merged), **E15** (a second instance of the same server-side attestation), **E17** (22 → 23 prose files) and **E21** ([[DEC-0038]] is now the highest landed decision record). **Not re-probed and still `UNKNOWN`:** U1–U6 — re-probing any of them would still require the forbidden action that made it `UNKNOWN` (§3.3). **One citation outside §3 also moved and is repaired at §6.2:** `product-orchestrator-agent.md`'s *Papéis operacionais* row and its `READY FOR PRODUCT LEAD REVIEW` clause are at **`:399`** and **`:686`** at the reconciliation base, having been `:397` and `:684` at the initial base, because [[DEC-0038]] §14 added two lines above them. **Their text is byte-identical; only the line numbers moved.**
...
| E14 | **PRs #86, #87 and #88** — the three most recent governed landings **at the initial base**, including `DEC-0037`'s own — each have `user: AdeptLabsDev`, `merged_by: AdeptLabsDev`, **`reviews: []`**, `requested_reviewers: []`, and zero comments. **`RE-DERIVED @ 1e7e124a`:** PR **#89**, which landed [[DEC-0038]], has since merged with **exactly the same shape** — `user` and `merged_by` both `AdeptLabsDev`, `reviews: []`, `requested_reviewers: []`, zero comments — so the three most recent governed landings are now **#87–#89**, and the finding is **strengthened, not disturbed**. **This is not a defect in PR #89 or in any of the four**: their independence was process-side and none claimed otherwise (§13). | `gh api repos/.../pulls/<n>` and `/reviews` |
| E15 | Merge commits are created server-side: `4566738` has `committer_login: "web-flow"` and `verification.verified: true` / `reason: "valid"`; **`RE-DERIVED @ 1e7e124a`** — the reconciliation-base merge commit reads identically (`committer_login: "web-flow"`, `verified: true`, `reason: "valid"`, `author_login: AdeptLabsDev`), a second instance of the same pattern rather than a new fact. Branch commits are `verified: false` / `reason: "unsigned"`; local signing config (`commit.gpgsign`, `gpg.format`, `user.signingkey`) is **unset**. | `gh api repos/.../commits/<sha>`; `git config --get` |
...
| E17 | `@noxund/orchestrator` is referenced by **22 files outside its own package, all under `docs/**` and all in prose**, by **zero** executable files outside the package, and by **zero** workflows. **`RE-DERIVED @ 1e7e124a`: the prose count is now 23**, the single delta being [[DEC-0038]]'s own record — the two clauses [[DEC-0038]] §14 repaired in `product-orchestrator-agent.md` still name the package inside their reconciliation notes, so that file stays in the set. **The count is base-scoped and this record's own landing advances it to 24, this record being itself one such prose file** — a literal number moving, not a substantive change. **No conclusion here rests on the count**, as E13 says of its own: **the zeroes are unchanged and are the load-bearing part.** **Stated more precisely than the inherited phrasing:** [[DEC-0037]]'s evidence base records it as appearing *"in no file outside its own package"*; read as the **executable-reachability** claim it was made for, that holds exactly; read literally it would be falsified by documentation prose. The substance is unchanged — nothing executable reaches it. **This is a capability fact only; the disposition of `packages/orchestrator` is `C2`'s and is not touched here.** | `git grep -l "@noxund/orchestrator"` with path exclusions |
...
| E21 | **`RE-DERIVED @ 1e7e124a`. [[DEC-0038]] is the highest landed decision record on `main`** — at the initial `C3` evidence base it was `DEC-0037`, `C2`'s landing (PR #89) is what changed it, and **this record's own landing advances it again, to `DEC-0039`** — the row states what was true at each named base and is never a durable ranking. **No `DEC-0039` file exists on any local or remote ref** other than this record — the collision-freedom this record's own identifier depends on, and a claim that **cannot decay**: a later `DEC-0039` elsewhere would be another unit's defect, not staleness here. **The claim is scoped deliberately.** An earlier revision of this row asserted the same of `DEC-0038` across all refs; that assertion was **true when made and is no longer true**, because unmerged branch refs move independently of `main`. **That recorded self-correction stands unaltered, and the post-`C2` base has now overtaken it in the other direction as well:** `DEC-0038` is no longer merely on a branch ref — it is landed on `main`. Both facts are kept; neither is erased. **Nothing in this record rests on, characterizes or takes authority from any unlanded ref** — an unlanded artifact binds nothing ([[DEC-0035]] §3.2, §6), and landed authority is what this row is about. | `git ls-tree -r HEAD -- docs/product/decisions/`; `git ls-tree` over `git for-each-ref refs/heads refs/remotes` |
...
The `C2` consequences are set out in §7 and are binding as part of this decision. **`C2` is decided and landed: [[DEC-0038]] chose `REDUCE`, and `C3` neither decides, revisits, reopens nor weakens it.** This record was built to hold under all three dispositions and did — that property is now **historical**, discharged by `REDUCE` landing without forcing any change to D1–D16 (§7).
...
| **#14–#16** | **LATENT** — the `C2` condition is now resolved | they trigger on control-plane changes to `@noxund/orchestrator`, whose author identity is `CONTRACT-PROPOSED / RUNTIME-NOT-WIRED / NOT-OPERATIONAL` and whose package is reached by no executable file and no workflow (E17). **Landed [[DEC-0038]] D1/D3 settles the condition: the control plane is reduced to non-authoritative legacy, no code changes, and no runtime is wired — so no trigger fires and they remain LATENT.** They would become LIVE only if a separately authorized future unit changed control-plane code, at which point #15's `security_agent` requirement meets the same P4 divergence §6.3 flags. See §7 |
...
## 7. `C2` interface — the disposition is landed, and it is **REDUCE**

**Current authority.** [[DEC-0038]] is landed on `main` at `1e7e124a` (PR #89, 2026-08-21) and decides `packages/orchestrator`'s disposition: **REDUCE**. That is the operative path and the only one. **`C3` does not decide, revisit, reopen or weaken it**, and nothing in this section may be read as re-deciding a disposition another landed record holds.

**What the earlier three-branch construction was, and what it is now.** This record was drafted while `C2` was in flight and was deliberately built to hold whichever disposition `C2` returned — it carried the line `THIS RECORD IS VALID UNDER ALL THREE DISPOSITIONS`. **That was a true design property and it discharged its purpose:** nothing in D1–D16 had to change when `REDUCE` landed, and the model below is the same model, now stated against an actual disposition rather than three hypotheticals. **It is recorded as history, not as three live options.** `RESUME-AND-HARDEN` and `RETIRE` are **not currently possible outcomes**; [[DEC-0038]] §11 states the grounds on which each was rejected. Where either limb carried a still-useful conditional it is resolved in place rather than left hanging — matrix items #14–#16 at §6.4, and the runtime-approver standing rule at §7.3.

**The inherited authority direction is unchanged.** [[DEC-0037]] §9 fixes `C1 GOVERNANCE MODEL → RUNTIME IMPLEMENTATION`, never the reverse. A runtime **implements** this model; no runtime behaviour amends it. [[DEC-0038]] §15 is an instance of that direction, not an exception to it.

### 7.1 What `REDUCE` retains — [[DEC-0038]] D2's inventory, consumed exactly

Paths are relative to `packages/orchestrator/`.

**Retained — `CANDIDATE PRIMITIVE · NON-AUTHORITATIVE · NOT ADOPTED` — these four and only these four:**

| # | Retained primitive | Where it exists |
|---|---|---|
| 1 | **Command identity / JCS canonicalization** | **Preserved only** — `151fb46:src/core/command-identity.ts`. **Not on `main`** |
| 2 | **Approval binding and verification semantics** | **Preserved only** — `151fb46:src/core/safety.ts`. **Not on `main`** |
| 3 | **Single-use consumption-ledger semantics** | **Preserved only** — `151fb46:src/core/project-state.ts`. **Not on `main`** |
| 4 | **Safety classification** — `assessSensitivity`, the sensitive-action set, the destructive-payload heuristics | **LANDED on `main`** — `src/core/safety.ts` |

**Reduced to legacy — `NON-AUTHORITATIVE · LANDED · UNCHANGED · NOT THE INTENDED ARCHITECTURE`** ([[DEC-0038]] D2): `src/core/orchestrator.ts` (the run loop and `bootstrap()`); **`src/core/dispatcher.ts` (routing)**; **`src/core/agent-registry.ts` and all ten `src/agents/*`**; `src/core/decision-schema.ts` and `src/core/decision-validator.ts`; the project-state reducers and store in `src/core/project-state.ts`; `src/core/logger.ts`; `src/core/ids.ts`; `examples/delegate-task.ts`.

**A correction to this record's own earlier revision, recorded rather than quietly overwritten.** While `C2` was in flight, this section's `REDUCE` limb predicted that the primitives carrying principal semantics would be *"the human-approval gate (`requires_human_approval`), the dispatcher's authorization check, the agent-registry allow-list, and any binding of an approval to a task."* **Two of those are reduced to legacy by [[DEC-0038]] D2 and are not retained at all** — the dispatcher's routing and the agent registry. The table above is [[DEC-0038]]'s inventory, not a prediction, and it replaces that guess.

**Routed elsewhere, and not a `C2` primitive:** `src/core/result-schema.ts` and the `AgentResult` envelope are **`C4`'s** subject ([[DEC-0038]] D2, §15). This record takes no position on them beyond §8.

**Retention is not adoption, and `REDUCE` is not a deletion instruction** ([[DEC-0038]] D1, D3). Three of the four exist **only** at `151fb46` and are **not on `main`**; the fourth is landed and is reached by no executable file and no workflow (E17).

### 7.2 What each retained primitive does — and does not — answer

**None of the four supplies approver provenance.** [[DEC-0038]] §13 states this of itself — *"no primitive retained in D2 may ever be represented as supplying provenance"* — and this record **agrees with it**, rather than merely avoiding contradiction.

| Retained primitive | What it answers | What it does **not** answer |
|---|---|---|
| **COMMAND IDENTITY** | *What exact command is authorized?* Content-addressed, deterministic across processes, domain-separated | **It does not authenticate who approved it.** A hash names a payload; it makes no claim about a party |
| **APPROVAL BINDING / VERIFICATION** | Whether an approval corresponds to an expected command representation — minted against a concrete command, verified against a live recompute | **It still cannot prove the claimed approver was actually the Product Lead**, absent an independent trust anchor. It proves correspondence, never authorship |
| **APPROVAL CONSUMPTION LEDGER** | *Has this authorization already been used?* Durable, test-and-set, fail-closed when absent, fail-loud when corrupt | **It does not authenticate who issued it.** Single use is a count, not an identity |
| **`assessSensitivity`** | How sensitive an action is, against a fixed action set and destructive-payload heuristics | **It supplies no approver provenance whatever**, and [[DEC-0038]] §18 item 4 records that its heuristics are regex-based and evadable |

```txt
IDENTITY ≠ CONSUMPTION ≠ PROVENANCE
HASHING IS NOT ATTESTATION
```

**Any future unit that adopts one of these primitives inherits D6's limitation and must state it in its own terms.** A gate built from them is an **interaction** gate — it constrains *what* and *how many times* — and **may not be described as verifying a provenance it cannot verify**. Primitives that are pure schema validation, state tracking or logging carry **no** principal semantics and inherit nothing from this record.

### 7.3 The approver-provenance handoff, answered

[[DEC-0038]] §13 and §15 record `APPROVER PROVENANCE = OPEN · ROUTED TO C3`. This record answers it:

```txt
TECHNICAL APPROVER-PROVENANCE = NOT CURRENTLY SEPARABLE FROM SINGLE PRINCIPAL
```

**The answer is adjudication, not implementation** — the sense in which the header's *Discharges* clause is meant. The reason is structural rather than incidental: the approval, the artifact and the acting agent all originate from one account and one host session (E2, E3, E14, E18). The principal-separation decision is **P3 — HYBRID** (D9), unchanged by `C2`'s landing.

**The prerequisite, stated exactly and provisioned by nothing here.** For any **future non-human consumer** that must rely on an approval being genuinely Product-Lead-issued, **a trust anchor whose private half is inaccessible to the acting agent is a prerequisite for that reliance** — not for the approval itself, and not for any work that does not rely on it. D6 names the three categories of such a mechanism and designs none. **`C3` provisions nothing:** no App, bot, account, token, credential or signing key is created, and no mechanism is selected.

**What `REDUCE` changes for the runtime-approver limb: nothing — and that is the finding.** Under [[DEC-0038]] D1 no runtime is wired and nothing consumes an approval, so the limb has **no present referent**. It survives as a standing rule: **any future automated approver requires a principal distinct from the acting one before its approval may be relied on** (D9 item 3). Everything in D1–D12, F4a, F4b, the P3 decision and the §5 capability surface concerns **human and GitHub** principals and stands untouched and fully in force. **Reduction removed an implementation, not a principle.**
...
[[DEC-0035]] §10 requires the forward-discoverability update to be created **in the same authorized unit** that creates the relation. **Three** edges this record creates are recorded in the relation table at `docs/product/context-map.md` §3:
...
3. `DEC-0039 DISCHARGES DEC-0038 §13 · §15` — **by satisfaction, not by replacement** — answering the `APPROVER PROVENANCE = OPEN · ROUTED TO C3` assignment. **`DISCHARGES` is the deliberate token**: the routed item was a *question assigned for adjudication*, and it is discharged by being adjudicated rather than solved — the distinction [[DEC-0038]] §3 draws about its own discharge of [[DEC-0032]] §8. **`EXTENDS` would misdescribe an answered assignment as a supplied limb, and no supersession token applies**, [[DEC-0038]] being neither edited nor narrowed and its `REDUCE` disposition neither rewritten nor reopened. **[[DEC-0038]] is FROZEN and is not edited**; the external table is the mechanism [[DEC-0035]] §10 directs for that case.
...
1. **`docs/product/context-map.md`** — the **three** §3 relation rows required by §9; **two** §1 routing rows — one for the capability and principal model, one routing `packages/orchestrator`'s **REDUCE** disposition to [[DEC-0038]] so a fresh operator reaches it directly; a pointer on the existing `docs/agents/**` row; the `C*` lifecycle cells in §2, which now carry `C4` = the result / disposition contract and `C5` = mechanical enforcement as **NOT STARTED / NOT AUTHORIZED** destinations; and a two-base scoped-reverification paragraph per that document's convention. **No `C4` or `C5` GO is created by any of it.**
2. **`docs/product/current-state.md`** — §E's *"Process independence is not mechanical independence"* bullet is repaired where this unit's live read makes its provenance stale (the claims are now `VERIFIED`, not `REPORTED`) and where new verified protections are added; §B's Axis-2 ladder and authority paragraph are repaired because **this unit's own landing** makes the *"not started"* statement false for `C3`. A two-base scoped-reverification paragraph is added.

**Post-`C2` reconciliation, performed in this round.** [[DEC-0038]] landed on `main` between this record's initial evidence base and its landing, and rewrote the same `current-state.md` §B and `context-map.md` §2 cells this unit edits. The collision was resolved by **integrating alongside, never on top of**: every landed `C2` statement — the `REDUCE` disposition bullets, the parallel-state correction paragraphs and the `DEC-0038` §3 relation rows — is **preserved**, and `C3`'s content is added around it. **No `C2` statement is reverted, weakened or contradicted, and [[DEC-0038]] itself is not touched in any respect.** Two pre-existing lines that `C2` had already replaced on `main` — the inherited `REPORTED`-provenance independence bullet and the *"substantive `packages/orchestrator` work remains `C2`"* line — are not reinstated; the former is superseded by this unit's measured version, the latter by [[DEC-0038]]'s `REDUCE` bullets. `docs/agents/product-orchestrator-agent.md` carries [[DEC-0038]] §14's two repairs and is **not edited by this unit**, which touches no file under `docs/agents/**`.
...
- **It does not decide, revisit or reopen `C2`** — `packages/orchestrator`'s disposition is decided by landed [[DEC-0038]] (`REDUCE`), and this record neither rewrites nor supersedes it. It takes no position on runtime hardening or runtime-specific approval mechanics beyond §7's statement that no retained primitive supplies provenance.
...
- it **does not retroactively validate or invalidate any earlier unit**, including its own production, which stands on the `C3` GO and on nothing in this record — **a record does not ratify itself**. In particular, PRs #86–#89 (E14) are **not** retrospectively defective: their independence was process-side, and no unit claimed otherwise;
