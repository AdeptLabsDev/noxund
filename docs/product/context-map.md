...
| Scope admission; `OD-01`…`OD-07` | `docs/product/scope-guardrails.md` | **class not adjudicated** — no class is assigned here or by `DEC-0035` §9, which flagged it. *Status:* `HOLD — PRODUCT-LEAD ADJUDICATION REQUIRED`, B3 and B5 both having declined (see below) |
...
**Two adjudications `DEC-0035` routed here. B3 declined both; B5 reports where each now stands and adjudicates neither** — deciding either on this surface would create classification authority this artifact disclaims in its header.
...
- **The class of `docs/product/scope-guardrails.md` (Annex A row 12) — `HOLD — PRODUCT-LEAD ADJUDICATION REQUIRED`.** `DEC-0035` §9 withholds the `docs/product/*.md` family default from that one file and states its class *"requires explicit B3 adjudication"*. The class does **not** follow mechanically from landed authority: the §*Source of Truth* hierarchy ranks `/context` files and this is not one, while `docs/agents/product-orchestrator-agent.md`:167 incorporates its out-of-scope list by reference (*"Lista completa de fora-de-escopo em `docs/product/scope-guardrails.md`"*) and `agent-registry.md`:7 points at it — and whether that reference confers derived weight on the whole file, on one section, or on nothing is precisely the judgment `DEC-0035` reserved. Note the discipline that makes this non-obvious in both directions: imperative wording confers no authority (`DEC-0035` §3.1), and a file is never promoted because a clause of it is relied on elsewhere (§9.1). **Escalated to the Product Lead, unresolved.** B5 assigns no venue — that remains a Product-Lead routing decision.
