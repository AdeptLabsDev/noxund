...
| `P0` … `P10` (PG-EXIT) | PHASE | PG-EXIT program (Axis 1) | — (label) | P0/P1 closed by `DEC-0028`; P3 series adjudicated (next row); P9 not started; P10 has not occurred | plan table at `DEC-0028` §9 (P2–P10); P0/P1 in the `DEC-0028` header | Status of **P2 and P4–P8 is not established by any landed record indexed here — UNKNOWN**. Collides by shape with `P5-REPRO-01` and with `Fase N`. |
...
- **`SG-8` has two senses, both real.** (a) the **gate slot** `SG-8` in the channel-data checklist, whose content is `P5-REPRO-01` pre-publish; (b) shorthand for the larger **reproducibility work program** (`DATA-SG8-001`, `DEC-0024`/`0025`/`0026`, units `U1`/`U2`/`U3A-*`, migrations `0008`/`0009`). **`SG-8` was not terminally completed** in either sense.
...
- **Improvement Phase D/E/F are Product-Lead program definitions**, not independently landed phase records. A–C are landed.
...
| `stash@{0}` — local stash entry | — | 1 entry | On `feat/sg8-reconciliation-session-schema`: PR #57 traceability, the `DEC-0024` ordering-policy addendum, held until a PR #57 update is authorized |
| `AdeptLabsDev/noxund-unitd-x1b-run-30450421392-preservation` — external repository | `be83d45` | 11 | **D-X1b**: the Actions logs of run `30450421392`, preserved outside finite Actions retention. `PRIVATE` · `ARCHIVED` · not a fork · single root commit, 0 parents · **owner-reversible**, never "immutable" (`DEC-0033` §4) |

**Three preserved classes, per `DEC-0035` §9.** Git refs, the local stash entry, and the external preservation repository — all three are listed above. `DEC-0033` §8 expressly forbids converting the D-X1b repository into an adoption or into any product, runtime, build or CI dependency.
