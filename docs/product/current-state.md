...
**Authority:** **each Phase-C unit requires its own explicit Product-Lead GO**; the C1 GO authorized C1 and the C2 GO authorized C2, and each expired with its unit (`DEC-0035` §6). **Landing C1 and C2 authorizes nothing further** — not `C3`…`C5`, not any implementation, adoption or deletion of `packages/orchestrator` code (`DEC-0038` D3, §16), not any Axis-1 resumption, and no re-arming (`DEC-0033` §8). **Closing Phase B likewise authorized nothing.** No successor unit is authorized on either axis at this base.
