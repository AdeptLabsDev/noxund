...
- **C — agent-governance / runtime redesign:** **STARTED, not complete.** Its landed units are **C1**, the execution topology ([`DEC-0037`](decisions/DEC-0037-execution-topology-role-independence-governance-review-function.md)), and **C2**, the `packages/orchestrator` disposition — decided **REDUCE** ([`DEC-0038`](decisions/DEC-0038-packages-orchestrator-disposition-reduce-to-approval-integrity-primitives.md)). **C3** — the unit `DEC-0037` §9 reserved for the capability matrix — **EXECUTED IN PARALLEL with C2, under its own separate and explicit Product-Lead GO**, producing **PR #90 (OPEN, UNMERGED)**; its **final disposition is PENDING GOVERNANCE REVIEW** and is deliberately **not characterized here** — neither its content nor its outcome is asserted, and no record it proposes is on `main`. **C4 · C5** are Product-Lead-defined unit labels, **NOT STARTED / NOT AUTHORIZED** (`DEC-0037` §9). Its predecessor `C0` was an unlanded in-session assessment with **no file on `main`**, carrying a historical `RED` disposition adjudicated `CLOSED — REMEDIATED` — never routed from as clean evidence (`DEC-0037` header).
- **D · E · F:** **NOT STARTED**
...
**Authority:** **each Phase-C unit requires its own explicit Product-Lead GO**; the C1 GO authorized C1 and the C2 GO authorized C2, and each expired with its unit (`DEC-0035` §6). **Landing C1 and C2 authorizes nothing further** — not `C3`…`C5`, not any implementation, adoption or deletion of `packages/orchestrator` code (`DEC-0038` D3, §16), not any Axis-1 resumption, and no re-arming (`DEC-0033` §8). **Closing Phase B likewise authorized nothing.** **State, corrected in R2:** `C3` **was** separately and explicitly authorized by the Product Lead and executed in parallel with `C2` — that GO was its own and derived nothing from `C1`, `C2` or this document. **Beyond it, no further successor unit is authorized on either axis at this base**, and no Axis-1 unit is authorized at all.
