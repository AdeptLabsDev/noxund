...
## 12. Adendo (2026-07-15) — SG-V7 EXECUTADO E FECHADO; `run_id` congelado; A7 rotação GREEN; próxima fronteira = SG-6

- **O SG-V7 foi executado e COMPLETADO em 2026-07-15**, sob cadeia humana integral (GO de ciclo, dispatch, A3, A4 — todos do Product Lead; agentes read-only). Foram necessários **três dispatches**: os runs `29389945976` (falha limpa — driver psycopg não provisionado; corrigido no **PR #45**, merge `07f58b4`) e `29444609139` (falha limpa — projeto Supabase **auto-pausado** desde 2026-07-11; restore manual do Product Lead) falharam **pré-criação com raio de dano zero** (0 quota, 0 `run_id`, 0 escrita) e estão **preservados** (nunca re-executados); o run **`29446423135`** completou **success 3/3**. Registro completo: `docs/data/SG-V7-data-collect-001-first-collection-closeout.md` (checkpoint **A6**).
- **`run_id f0485de6-0d34-41cf-ab48-d46e483aa558` — CONGELADO, §7-PASSED**: 500 vídeos (`target_reached`), 12 páginas `search.list` + 10 batches `videos.list`, **1.210 unid** de quota (0 retry surplus; nenhum threshold DEC-0020 cruzado), veredito §7 integral no log do verify. `F2'-N1` (run_id → `$GITHUB_OUTPUT`) **provado em produção**.
- **A7 (rotação staged pós-run da `YOUTUBE_API_KEY`) fechou ✅ GREEN em 2026-07-15** — registro em `docs/security/SEC-0026-a7-key-rotation-closeout.md` (E7a–E7f): key nova com restrições idênticas, antiga excluída pós-validação, inventário final = **1 key ativa**, próximo deadline ≈ **2026-10-13**.
- **Risco operacional registrado (mitigação em aberto):** auto-pause do Supabase free tier; check de **liveness** do projeto passa a ser **requisito das revalidações pré-dispatch futuras**. Pendência documental separada: host do pooler (`aws-0` vs `aws-1`) em INFRA-0001/0002.
- **A próxima fronteira é o SG-6 (Channel Data / 002)** — consumirá o `run_id` congelado sob novo ciclo humano completo. **NO-GO até ordem explícita do Product Lead.** Publish segue barrado até SG-8/P5-REPRO-01; `0007` PARKED; Fase 9/RLS vetada.
