...
> **Modelo provider-neutral que passa a valer (com EMENDA DE COMPLETUDE):** a proveniência por rodada é de **COMPUTAÇÃO DETERMINÍSTICA** — **3 colunas** `compute_engine_name`, `compute_engine_version`, `compute_manifest_hash` — **obrigatória e SIMÉTRICA nas 2 rodadas**, FORA do digest. O `compute_manifest_hash` é o SHA-256 (64-hex) da serialização canônica de **TODOS os determinantes computacionais**: `manifest_format_version` + `engine_name` + `engine_version` + `pipeline_version` + `resolver_version` + `rule_version`/`rule_hash` + `rubric_version`/`rubric_hash` + `opportunity_version`/`opportunity_hash`. **NÃO** existem mais `compute_adapter_version` (identidade de **persistência**, não participa do cálculo) nem `compute_params_json` (metadata ambígua; todo knob que altera o resultado já vive numa config versionada). **SEM** `llm_*`, **sem** `ext_llm_*`, **sem** `engine_kind='llm_assisted'`. O **PASS gate** exige, além dos digests R1==R2, **equivalência computacional R1==R2** em `compute_engine_name`, `compute_engine_version` e `compute_manifest_hash` (qualquer divergência ⇒ FAIL mesmo com digests iguais). Uma futura assistência por modelo seria uma feature **separada, não-autoritativa, desligada por padrão**, sob **nova decisão** — não pré-modelada aqui. Detalhe canônico do manifesto: `sg8_coordinator.canonical_compute_manifest`. Ver `docs/product/decisions/DEC-0025-sg8-llm-decoupling.md` (§EMENDA DE COMPLETUDE DA PROVENIÊNCIA).
>
> **CONTRATO DE COMPARAÇÃO (emenda posterior · DEC-0025 §EMENDA — CONTRATO DE COMPARAÇÃO VERSIONADO):** `sg8_sessions.comparison_contract_version` (`text NOT NULL`, sem default, imutável, EXCLUSIVAMENTE `sg8-pass-v1` nesta migration; constante de código `SG8_COMPARISON_CONTRACT_VERSION`) identifica a **SEMÂNTICA do gate que JULGA Round 1 vs Round 2**. Distinção canônica e **inegociável**: **`compute_manifest_hash` = identidade das CONDIÇÕES que PRODUZEM o resultado**; **`comparison_contract_version` = identidade das REGRAS que JULGAM R1 vs R2**. O contrato de comparação **NUNCA** entra no `compute_manifest_hash`, não produz score/digest e não é duplicado nas rodadas. O gate de PASS recusa avaliação sob versão ausente/desconhecida/diferente. Evolução: nova semântica ⇒ nova migration + novo identificador; `sg8-pass-v1` nunca reusado; sessões terminais preservam a versão sob a qual foram julgadas.
...
> **❌ SUPERADO por DEC-0025 (LLM decoupling) + EMENDA DE COMPLETUDE.** Esta seção descreve a proveniência LLM-específica (`sg8_round_executions.llm_*`, `prompt_hash`, provider/modelo) que **não existe mais**. Substituída pela **proveniência de computação determinística** — as **3 colunas** `compute_engine_name`/`compute_engine_version`/`compute_manifest_hash` (o manifesto incorpora `manifest_format_version` + identidade do engine + pipeline/resolver/rule/rubric/opportunity versions+hashes; **não** há `compute_adapter_version` nem `compute_params_json`) — **obrigatória e simétrica nas 2 rodadas**, com **equivalência computacional R1==R2** (engine name/version + manifest) exigida pelo PASS gate. Ver a EMENDA no topo e `DEC-0025` (§EMENDA DE COMPLETUDE DA PROVENIÊNCIA). O texto abaixo é preservado como registro histórico.
