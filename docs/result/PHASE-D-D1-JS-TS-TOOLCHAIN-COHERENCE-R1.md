...
**Author boundary.** This record states **no unit disposition and no artifact verdict of its own** ([`DEC-0040`](../product/decisions/DEC-0040-governed-result-disposition-closeout-contract.md) D11). Both are void coming from an Author.
...
> **STANDING RULE FOR THIS SECTION, adopted in R5 because three consecutive rounds each introduced a fresh count defect here.**
>
> 1. **R1's own acts are stated by the table immediately below, which R1 authored.** **Every round after R1 that writes to the repository OR executes anything adds its own block to §17.2** — writing without executing still qualifies. **The rounds named at §17.1 and at §17.2 must therefore match, R1 excepted**, and that correspondence is what makes an omission detectable rather than silent: a round that adds no block has not been accounted for, and the mismatch shows it.
> 2. **The table below states whole-unit invariants, attested only through the rounds that appear in §17.2** — a row here is never evidence about a round that had not happened when it was written, which is the *"true when written, incomplete afterwards"* failure §20.1 names.
> 3. **Counts that a later round can falsify are not written.** Where a count-free formulation carries the same meaning it is used instead — the discipline `DEC-0042` §5 adopts, which this record already applied to push counts and to head SHAs, and **failed three times to apply to file counts.**
>
> 4. **Every row states WHOSE acts it covers.** A row under a whole-unit heading that reports only the Author's acts is a **scope defect**, and no amount of counting detects it. **Where a unit-wide claim is falsified by the reviewer-side breach `G-1`, the row says so at the point of the claim and routes to §20** — a reader must be able to determine the current `DEC-0040` D13 accounting **from this table alone**, without reconciling it against another table in the same record.
>
> **The three COUNT failures, named rather than buried:** R2 wrote six edits where it had made seven; R3 left a four-file blob-identity list after landing a fifth file (`M-2`); R4 repaired that row but left *"the four files above"* in the row beneath it (`NEW-2`), left *"the four in the write scope above"* in §17.1 (`NEW-A`, found by R5's own sweep rather than reported to it), and wrote *"seven"* over an enumeration of eight (`NEW-4`). **Each round repaired only the count it was told about. R5 swept the whole section instead.**
>
> **The SCOPE failures — a different class, which the count sweep could not catch and did not.** R5 verified every number here and every number was right; **what was wrong was whose acts some of these rows described.** The table promised whole-unit invariants and delivered the Author's, so **`G-1` — a reviewer-side write outside `write_scope`, inside this governed unit — was recorded correctly at §20 and contradicted here.** **Repaired throughout in R6**, and rule 4 above exists so the class is closed rather than moved.

| Surface | Finding | Class |
|---|---|---|
| **Repository files** | **Exactly FIVE created or modified across the whole unit**, all inside the declared write scope: `pnpm-workspace.yaml`; `apps/web/tsconfig.json`; this record; `current-state.md`; `context-map.md`. **`M-2`, repaired in R4: this cell opened "exactly four" and then listed `apps/web/tsconfig.json` as a fifth two sentences later — a self-contradiction inside the `DEC-0040` D13 accounting surface, in a record that lands `FROZEN`.** **R2 touched two of the four then in scope and created nothing new** — this record and `pnpm-workspace.yaml`; `current-state.md` and `context-map.md` were **not** reopened in R2, the Reviewer having verified both as accurate and minimal (§17.1). **R3 raised the total to five** by adding **`apps/web/tsconfig.json`**, authorized by the Product Lead's explicit R3 scope extension and by nothing earlier (§19). **`apps/web/README.md` was NOT touched** — the grant permitted that file **or** the config, not both, and T1 needs only the config. Checked by `git status --porcelain` in the primary checkout and by `git diff --name-only` across each commit and across `3110acff..HEAD`. **`pnpm-lock.yaml`, root `package.json`, every workspace manifest and `.nvmrc` are blob-identical to the base** (§8, §12). **`apps/web/tsconfig.json` is NOT — it was deliberately changed in R3** (§19): base `b8cdcc79e815a04397e53596f549d737f0b55738` → head `cfdac2ba59a356647b650a9471c2eb522f6f970f`. **R3 left it inside this blob-identity list, where it had been true through R2 and was false the moment T1 landed; `M-2`, repaired in R4** | `AUTHORIZED MUTATION` |
...
| **Commits / pushes / pull requests** | **One branch, one pull request, left UNMERGED.** Every commit sits on that branch and touches only files in the write scope enumerated in the row above; every push targeted that branch alone. **`NEW-2`, repaired in R5: this cell read "the four files above" — jointly wrong with the row above at `0fb90b5`, which was `M-2`; R4 repaired that row and left this one, making the table self-contradictory across neighbouring rows. It is now count-free, so no later round can falsify it by adding a file.** **No count is stated, deliberately** — a correction round adds a commit and a push, so a number written into a `FROZEN` record is falsified by the act of writing it (the rule `DEC-0042` §5 adopts, and the defect `D0` R4 had to repair in its own predecessor). Checked by `git log 3110acff..HEAD` with `git show --name-only` per commit; by `git branch -a --contains` on the first commit; by the branch reflog — **created from the canonical base, every entry a plain `commit:`, no amend, reset, rebase or force update**; and by `git for-each-ref refs/remotes/origin`, where `origin/main` still reads `3110acff…` | `AUTHORIZED MUTATION` |
| **Refs, tags, stash** | **GENUINELY UNIT-WIDE — checked on the shared repository, so it reaches reviewer-side acts as well as this Author's. None created, deleted or moved that survives.** Checked by `git for-each-ref`, `git tag --list` and `git stash list` — **one pre-existing stash entry, untouched**. **Stated completely rather than as a bare negative: the reviewers created their own separately authorized worktrees, whose administrative registrations existed and were removed; `git worktree list` shows only the three pre-existing entries and `.git/worktrees` holds no stale registration** (§18.2). Any `refs/codex/…` harness refs present are outside this agent's control | **`ZERO MUTATION` unit-wide**, on that named shared-repository basis · harness refs `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL` |
| **Worktrees** | **Created and destroyed within the round that needed it, each authorized in advance by the writ. This row states the AUTHOR's R1 worktree; the Author's later rounds are at §17.2, and no round leaves one standing. The reviewers created their own separately authorized worktrees, which are the reviewers' to account for and are reported at §18.2 as theirs** (`SCOPE-D`, scoped in R6). `C:/Adeptlabs/noxund-d1-install-validation-author`, added detached at `3110acff…`, then moved to `4b54de0b…` and to `434411d7…` by `git checkout --detach` for the later probes — **one authorized path reused for all three probes, accounted for here rather than left implicit.** Checked by `git worktree list` before and after; the three pre-existing worktrees — `noxund` itself, `noxund-design-ref`, `noxund-p3c-execute-pre` — **were not touched. Cleanup did not go as scripted, and the deviation is reported rather than smoothed:** `git worktree remove --force` **deregistered the worktree but exited 255 with `error: failed to delete '…': Filename too long`**, leaving the directory on disk — the same Windows path-length limit that produced the point-8 warnings, hit here on the nested `@typescript-eslint` paths under `node_modules/.pnpm/`. `git worktree prune` then cleaned the administrative entry, and **the directory was deleted with PowerShell `Remove-Item -Recurse -Force` against the `\\?\` long-path form of the same path.** Confirmed by `Test-Path` returning `False` and by `git worktree list` showing only the three pre-existing entries. **Nothing outside that one directory was targeted or removed.** **R3 repeated this pattern with its own separately authorized pair and the identical failure and fallback — see §17.2** | `AUTHORIZED MUTATION`, created **and** cleaned up |
| **Out-of-repository files — the pnpm store** | **Created and then deleted, authorized in advance; this row states the AUTHOR's R1 store, later Author rounds at §17.2, reviewer-side stores at §18.2 as the reviewers' own:** `C:/Adeptlabs/noxund-d1-pnpm-store-author`, passed explicitly via `--store-dir` on **every** install so that no shared or default pnpm store was written. Deleted after the evidence was recorded, by the same `\\?\` long-path `Remove-Item` used for the worktree; confirmed absent by `Test-Path`. A listing of `C:\Adeptlabs` afterwards shows **neither authorized artifact remains**, and the many pre-existing `noxund-p3c-*` sibling directories from earlier PG-EXIT work were **neither read, modified nor removed**. **Cleanup here is completion of an authorized instruction, not concealment** — `DEC-0040` D13's *"cleanup is never retroactive authorization"* is respected because both writes were authorized **before** they occurred, are disclosed in full, and the evidence they produced is recorded above rather than destroyed with them. **R3's own store is accounted for at §17.2** | `AUTHORIZED MUTATION`, created **and** cleaned up |
| **Dependency materialization** | **THREE THINGS, separated in R7 because one row was carrying all of them.** **(i) UNIT-WIDE PRIMARY-CHECKOUT INVARIANT — `ZERO MUTATION`.** **No participant** created or refreshed `node_modules` in the primary governed checkout `c:\Adeptlabs\noxund`. The **four pre-existing trees there — including `packages/orchestrator/node_modules` — were not deleted, not refreshed, and were NOT used as reproducible-install evidence.** Checked by `git status --porcelain --ignored=traditional`. **(ii) AUTHOR MATERIALIZATION.** This Author's occurred **only inside its explicitly authorized validation worktrees and stores** — three trees (`.`, `apps/web`, `packages/shared`) per worktree — and every one was cleaned up as recorded at §17.2. **(iii) REVIEWER MATERIALIZATION.** The independent reviewers used **their own explicitly authorized worktrees and stores**, recorded at §18.2 as theirs; the fresh Reviewer materialized the same three trees and 320 packages, twice. **This Author's validation worktree was NOT the only dependency-materialization surface in the unit, and R1 through R6 left this row implying it was.** **One precise correction the earlier wording got wrong:** it said the pre-existing trees were *"not read as proof of anything"*, which **R4 falsified** — R4 performed **one disclosed read** of the pre-existing `apps/web/node_modules/typescript@5.9.3` **solely as compiler-default evidence** (`defaultValueDescription`, §19.4), which is **not** reproducible-install evidence and never stood in for one | **(i) `ZERO MUTATION` in the primary checkout**, on the named basis · **(ii) `AUTHORIZED MUTATION`** inside the Author's authorized surfaces, cleaned up · **(iii) the reviewers' own, at §18.2** |
| **Temp, cache, sidecar, backup files** | **AUTHOR ACTS — none created anywhere outside the authorized surfaces.** *(`SCOPE-B`, found by R6's own sweep rather than reported to it: this row read as an unqualified whole-unit claim and is **falsified for the unit by `G-1`**, whose two `wsp_*.yaml` files were written to system temp outside any authorized surface. The row is now Author-scoped, and the unit-wide picture is the `/tmp` row above and §20.)* One build artifact — `apps/web/tsconfig.tsbuildinfo`, written by `tsc` because `apps/web/tsconfig.json` sets `"incremental": true` — was created **inside the validation worktree**, is gitignored (`.gitignore:27`, confirmed by `git check-ignore -v`), and was destroyed with the worktree. **No backup, sidecar or `.orig` file was written.** The one temporary edit made anywhere was the Option-B negation probe to `pnpm-workspace.yaml` **inside the validation worktree**, reverted immediately by `git checkout -- pnpm-workspace.yaml` and confirmed clean by `git status --porcelain` before the worktree moved on | `AUTHORIZED MUTATION` within the authorized worktree, fully disclosed |
| **`/tmp` and system temp** | **THREE DISTINCT THINGS, kept apart because R1 through R5 conflated them and the conflation made this row FALSE for the unit.** **(a) AUTHOR ACTS — `ZERO MUTATION`.** No command this Author issued in any round wrote to system temp: none targeted `/tmp`, `%TEMP%` or any system temp path, no `mktemp` was used, no redirection left an authorized surface, and no tool was invoked for the purpose of writing there. *(R1 said "Nothing written", which reached past that check; R2 narrowed it to the Author's own acts, and that narrowing stands — but it was left sitting under a whole-unit heading, which is the defect R6 repairs.)* **(b) REVIEWER-SIDE `G-1` — an AGENT WRITE and a BREACH, not housekeeping.** The R1/R2 independent Reviewer created **`wsp_46d83d4b….yaml`** and **`wsp_df558694….yaml`** under system temp, **outside its `write_scope`**, through a stray shell redirection. **Finding `G-1 · CLOSED — REMEDIATED`. Unit disposition `RED — VERIFIED AUTHORIZATION BREACH` — permanent, and never rewritten by that closure** (`DEC-0040` D7; §20). **Both files are preserved and untouched.** **(c) AMBIENT GUID `.tmp` POPULATION — harness, not agent.** Path-level checks found GUID-named `.tmp` files timestamped inside the unit window, among a population spanning days before it. **(c) IS NOT (b), and must never be read as covering it:** the `wsp_*.yaml` pair are agent writes and a breach; the GUID files are neither | **(a) `ZERO MUTATION` for this Author's acts**, on the named basis · **(b) `UNAUTHORIZED MUTATION` — `G-1`, `CLOSED — REMEDIATED`, historical unit `RED` (§20)** · **(c) `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL`** |
| **Session scratchpad** | **AUTHOR-SCOPED, and the scoping is the point.** **Not used at any point by this Author, and that much is provable at path level.** No scratch file, note, helper script, intermediate result or pull-request-body file was created. **The pull-request body was passed inline**, and every commit message was passed via stdin, not via a file. Checked by enumerating the session scratchpad directory: **`find … -type f` returns `0`** — the directory exists, created by the harness, and is **empty**. Re-verified at R6. *(`SCOPE-C`, found by R6's own sweep: a session scratchpad is **per-session**, so this check reaches the Author's own and no other. The R1/R2 Reviewer separately reported its scratchpad empty, recorded at §18 **as that Reviewer's report**; for the later reviewers this Author has **no observable path and states none** — `UNKNOWN`, never assumed clean.)* | **`ZERO MUTATION` for this Author's scratchpad**, on that named basis · reviewer scratchpads `UNKNOWN` to this Author |
| **Memory files** | **GENUINELY UNIT-WIDE, and this row says why rather than leaving it to be assumed.** The project memory directory is a **single shared path used by every session — Author and reviewer alike — so an mtime check there reaches reviewer-side writes as well as this Author's**, which is exactly what the scratchpad row above cannot do. **No memory file was written, of any kind**, inside the repository or outside it — **no `MEMORY.md`, no project or session memory file, no settings file, and no `CLAUDE.md`.** Checked at path level by mtime rather than asserted: the newest file under the project memory directory is `MEMORY.md` at **`2026-08-21 12:52:34`**, and `.claude/settings.json` at `2026-08-19 21:20:32` — **both predate this unit's window**, and the Reviewer independently reproduced that finding (§18). **R1's broader phrasing — that `C:\Users\Miguel\.claude\...` "was not written to" — reaches past that check and is narrowed here.** Several paths **under** that root do carry mtimes inside the window: `.last-cleanup`, and the `session-env`, `sessions`, `shell-snapshots` and `backups` directories. **None is a memory file, a settings file or an agent write** — they are harness housekeeping this Author neither chose, requested nor could prevent, and **naming them is not a concession that a memory write occurred; none did** | **`ZERO MUTATION` for memory content**, on the named mtime basis · the harness housekeeping paths `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL` |
| **External mutable systems** | **Zero writes, with one authorized read class.** No workflow was created, edited, enabled, disabled or dispatched; no ruleset, branch protection, required check, `CODEOWNERS`, Environment, secret, variable or repository setting was touched; no database, cloud or collection resource was reached; nothing was re-armed (`DEC-0033` §8; `DEC-0042` §D12). **EXTERNAL MUTATION — unit-wide: `ZERO EXTERNAL CONFIGURATION MUTATION`.** The classification was always right; only the completeness clause beneath it was part-scoped. **NETWORK READS — unit-wide, and R7 states them completely because R1 through R6 called registry access *"the only network activity"*, which was never true of the unit.** They comprise **package-registry retrieval of the artifacts the frozen lockfile already identifies**, during authorized install validation on **both** the Author's and the reviewers' surfaces; **and read-only GitHub API queries — `gh pr view 95`, issued by this Author in several rounds and by reviewers.** **No publication, no registry mutation, no upgrade lookup used as an implementation input, no telemetry or config change, no workflow dispatch, and no pull-request mutation by any reviewer.** The pnpm update banner was declined. **The only writes to a hosted system are this Author's branch pushes and the single `gh pr create`**, accounted for above | **`ZERO MUTATION` for external configuration — unit-wide**, on that named basis |
...
**Later rounds, same unit, same Product-Lead GO and same canonical base.** This record was produced in R1 and revised in the rounds below. **No round count is written into this sentence** — a further round would falsify one, which is the same self-falsification rule §17 applies to push counts and which R2 had to extend to head SHAs (§10 Probe C). **The canonical base never moved:** every round was authored and pushed against `main` @ `3110acff2225e5a5e8e2c5a5ccfebefaca4ec9ae`, and **no round of this AUTHOR's repository correction work touched a file outside the write scope enumerated above.**

> **The scope of that sentence is stated rather than left to a charitable reading, and its one exception is named rather than implied.** It covers **repository-file mutations by the Author**. **It does NOT mean no out-of-`write_scope` write occurred in this governed unit — one did.** `G-1`: the R1/R2 **independent Reviewer** wrote two files to system temp outside its `write_scope`, which is **`UNAUTHORIZED MUTATION`**, is **`CLOSED — REMEDIATED`**, and is why the unit's historical disposition is **`RED — VERIFIED AUTHORIZATION BREACH`** — permanent, never rewritten (§20). *(`SCOPE-E`, repaired in R6. R5 made this sentence count-free and left it reading as a universal claim about the unit; a reader should not have to infer that "round" silently meant "Author repository correction round".)* *(`NEW-A`, found by R5's own sweep rather than reported to it: this sentence read "outside the four in the write scope above" — a fourth instance of the same count defect, in the very paragraph that congratulates itself for avoiding a round count. It is now count-free.)*
...
| **R4** | **On the FRESH independent Reviewer's `ARTIFACT VERDICT = HOLD` at `0fb90b5`** — a different party from the R1/R2 Reviewer, which is excluded — carrying **two material defects and — in the Reviewer's own headings — seven non-material**, and finding **no authorization breach in R3 by either party**. **That report headed *"Non-material defects (7)"* and then added an eighth as an unnumbered note, so "seven" is faithful to its heading and "eight" to its content; this record uses EIGHT, because the enumeration below is what a reader checks a number against.** **Write scope: three files — this record, `current-state.md`, `context-map.md`. `apps/web/tsconfig.json` was NOT reopened; the T1 change stands as verified.** No install, worktree or store was required and none was created. **Execution WAS performed**, and R4's own accounting is at §17.2: this Author re-derived the two `apps/web/tsconfig.json` blob hashes, read `defaultValueDescription` from the installed `typescript@5.9.3`, enumerated the GUID `.tmp` population in system temp, and ran `grep -c` on root `package.json` — read-only, but execution. *(`NEW-1`, repaired in R5: R4 wrote "no execution was required and none was performed", which was false of its own round.)* **`M-1`** — `current-state.md`'s claim that `D1` repaired the workspace declaration *"and nothing else on the toolchain surface"*, falsified by the same document's own line attributing the tsconfig repair to `D1`. **`M-2`** — this table's Repository-files cell opened *"exactly four"* and listed `apps/web/tsconfig.json` among files *"blob-identical to the base"*, contradicting itself two sentences on and asserting a blob identity that R3's own commit had destroyed. **Both sat in surfaces this governance round exists to get right**, and both are repaired above. **EIGHT accuracy corrections folded in** because the record freezes on landing: the non-probative `.next` proof (`NM-1`), the five-versus-four option count (`NM-2`), the missing bound on what the green typecheck proves (`NM-3`), the over-broad *"closes the `D0` row 1 defect"* (`NM-4`), one surviving over-broad path-level negative (`NM-5`), the independence label not matching `DEC-0040` D10 item 4 (`NM-6`), stale-count-then-correct in two routing cells (`NM-7`), and the Author-boundary wording (`NM-8`). **Two forward observations recorded and NOT repaired** (§19.6). **Nothing substantive was reversed** — T1, OPTION B, pnpm 9.0.0, the Node policy, lockfile byte identity, the legacy exclusion, the historical `RED` and both closures all stand |
| **R5** | **On the fresh Reviewer's DISCHARGE of its R3 `HOLD`** — `M-1`, `M-2` and all eight `NM` items resolved, **no authorization breach in R4 by either party** — and its replacement of that `HOLD` with a **narrower one confined entirely to §17**. **Write scope: this record and one line of `current-state.md`. `context-map.md` was NOT reopened.** No install, worktree or store; execution read-only. **`NEW-1`** — R4 had no mutation increment at all, though it both wrote and executed; §17.2 now carries one, and a **standing rule** at §17 makes the omission of any future round's increment structurally visible instead of depending on a reviewer noticing. **`NEW-2`** — the Commits row still said *"the four files above"* beneath a row R4 had corrected to five, leaving the D13 table self-contradictory across neighbouring rows. **Four further items folded in**: the broken *"those four"* back-reference (`NEW-3`), *"seven"* over an enumeration of eight (`NEW-4`), a false `§21.2` cross-reference (`NEW-5`), and the word *"alone"* in `current-state.md` (`NEW-6`). **Beyond the reported items, R5 swept the whole of §17 as instructed and found one more of the same species** — §17.1's *"outside the four in the write scope above"* (`NEW-A`) — **repaired count-free, along with the whole-unit worktree and store rows, whose leading singular counts were true of R1 only.** **Nothing substantive was reversed**; §20 and §21 were not reopened |
| **R6** | **Authorized under a NEW explicit Product-Lead GO**, on the fresh Reviewer's **`ARTIFACT VERDICT = PASS`** at `83bdc56` with **no replacement `HOLD` and no new defect** — its independent §17 count sweep came back clean, confirming R5 closed that class rather than moving it. **The Product Lead then found two contradictions that had survived every round, the count sweep included, because they are SCOPE defects rather than arithmetic ones**: a *true-of-a-part* statement sitting where a *whole-unit* claim belongs. **Write scope: this record and `context-map.md`; `current-state.md` NOT reopened; no install, typecheck or package-manager probe, so every technical input stays byte-identical to the reviewed `PASS` head.** **(1)** `context-map.md`'s registry paragraph still described `D1`'s output as of R2 — the result record *"plus the repaired `pnpm-workspace.yaml`"* — while the routing row in the same document named both configuration files; repaired count-free. **(2)** §17's `/tmp` row claimed `ZERO MUTATION` for *"this unit's own acts"* while §20 recorded `G-1`, so **one record carried two irreconcilable D13 accountings**; the row now separates **Author acts** from the **reviewer-side breach** from the **ambient GUID population**, and **(c) is expressly barred from being read as covering (b)**. **§17.2's R2 block is split** into Author-correction and independent-review/governance increments, because the breach happened during R2 review and the single block was false read across the cycle. **§17.1's scope sentence is narrowed explicitly** and its sole exception named. **A new rule 4** at §17 requires every row to state whose acts it covers. **Beyond the reported items the sweep found five more of the same class**, all repaired: the temp/cache/sidecar row (`SCOPE-B`), the scratchpad row (`SCOPE-C`), the worktree and store rows (`SCOPE-D`), and the §17.1 sentence itself (`SCOPE-E`). **R6 also repaired the `Refs, tags, stash` row**, by making its unit-wide scope explicit on the shared-repository basis and matching its classification cell to that body — **recorded here in R7 (`DISC-1`), because §17.1 is this unit's durable correction ledger and anything missing from it stays missing.** Two rows proved **genuinely** unit-wide rather than needing narrowing, and now say why: **memory**, on the shared-path basis, and **`Refs, tags, stash`**. **No total is stated for this round's repairs**, deliberately — a later round adding one more would falsify it, which is the rule this section already applies to file and push counts. **Nothing substantive was reversed**; §20 and §21 were not reopened |
| **R7** | **Authorized under a NEW explicit Product-Lead GO**, on the fresh Reviewer's **`ARTIFACT VERDICT = PASS`** at `9e93161` — both R6 contradictions repaired, all five `SCOPE` findings correct, no new defect and no verified breach. **The Reviewer's own scope sweep then found three clause-level residuals, all non-material and none blocking merge, and the Product Lead elected to close them.** **Write scope: this record alone.** `current-state.md` not reopened; `context-map.md` unchanged; **no install, typecheck, build, worktree or store, so every technical input hashes identical to `9e93161`.** **`SCOPE-F`** — the Dependency-materialization row conflated a unit-wide primary-checkout invariant, this Author's materialization and the reviewers' own, and **implied the Author's worktree was the unit's only materialization surface**; the three are now separated, and the clause *"not read as proof of anything"* is corrected, because **R4 did read the pre-existing TypeScript package once, as compiler-default evidence rather than install evidence.** **`SCOPE-G`** — the External-systems row called registry access *"the only network activity"*, which was never true unit-wide; read-only GitHub API queries also occurred, on both sides. **The `ZERO MUTATION` classification for external configuration was always correct and is preserved** — only the completeness clause was part-scoped. **`DISC-1`** — R6's own repair of the `Refs, tags, stash` row was missing from this ledger and is added, **count-free**. **Beyond the three, the rule-4 pass over the whole-unit table found NOTHING** — `Repository files`, `Branches` and `Commits / pushes / pull requests` are shared-repository facts, unit-wide by construction, and the rest were already scoped in R6. **One inconsistency was found outside that table and repaired:** R6's own split of the R2 block into Author and review sides left the later round headings bare, so `R3`…`R7` now name their party explicitly (`SCOPE-H`). **Nothing substantive was reversed**; §20 and §21 were not reopened |
...
### 17.2 Per-round mutation increments

**Every round that wrote to the repository or executed anything carries its own accounting here rather than inheriting an earlier round's** — the standing rule at §17. **The rounds named here must match the rounds named at §17.1**, which is what makes an omission detectable rather than silent.

#### R2 — Author correction increment

**Documentation-only, and included because the rule above admits no exception for a round that merely writes.** **This block covers the AUTHOR's correction acts only; the review side of the same cycle is the block that follows, and the two must not be read as one.**

| Surface | Finding | Class |
|---|---|---|
| **Repository files** | **Two modified, none created** — this record and `pnpm-workspace.yaml`. `current-state.md` and `context-map.md` **not reopened** | `AUTHORIZED MUTATION` |
| **Execution** | **None.** R2 ran no install, no worktree, no store, no build, no typecheck and re-ran no probe | `ZERO MUTATION` |
| **`/tmp`, system temp, scratchpad, memory, worktrees, stores** | **Nothing created on any of them by the Author.** No redirection operator, `tee` or `mktemp`; the pull-request body had been passed inline in R1 and every commit message via stdin | `ZERO MUTATION` **for the Author's acts**, on that named basis |
| **External mutable systems** | **Zero configuration writes.** One commit pushed to the existing branch; **PR #95 open and unmerged** | `ZERO MUTATION`, on that named basis |

**No breach is recognized by this Author in R2.**

#### R2 — independent review / governance increment

> **`SCOPE-A`, added in R6.** The block above was true of the Author's acts and **false if read as the whole R2 cycle**, because **the breach happened during R2 review.** Splitting them is the only way the D13 accounting for this cycle is complete without contradicting §20.

| Surface | Finding | Class |
|---|---|---|
| **System temp — the breach** | The **R1/R2 independent Reviewer** created **two files outside its `write_scope`** — `wsp_46d83d4b….yaml` and `wsp_df558694….yaml` — through a **stray shell redirection left in a command from an earlier draft**. **Not the Author's act, and inside this governed unit** | **`UNAUTHORIZED MUTATION`** |
| **Conduct after recognition** | The Reviewer **recognized the breach contemporaneously** but **did not immediately stop substantive execution** — approximately **fourteen further read-only review operations** followed before stop and escalation. **No further mutation occurred** | **`G-1b`** — procedural |
| **Evidence handling** | The Reviewer **self-reported, did not delete the files, and they are preserved.** **Deleting them would have compounded the breach** exactly as it did in `C0`; **not deleting was the correct act** (`DEC-0040` D13, *"cleanup is never retroactive authorization"*) | **`PRESERVED`** |
| **Lifecycle and unit effect** | **`G-1` and `G-1b` are both `CLOSED — REMEDIATED`.** Closure **never rewrites** the historical unit disposition **`RED — VERIFIED AUTHORIZATION BREACH`**, which is **permanent** (`DEC-0040` D5, D7). **The technical artifact verdict `PASS` survives** under the unit/artifact separation the Product Lead accepted expressly (`DEC-0040` D14) | **historical `RED`, preserved — §20** |

**This Author caused none of the above, and states it as the adjudicated record rather than as a defence.**
...
| **Temp, cache, sidecar, backup** | One `apps/web/tsconfig.tsbuildinfo`, written by `tsc` inside the R3 worktree because `incremental: true` is set; **deliberately deleted twice before typechecking** so that neither green result could be a cached pass, and destroyed with the worktree. Gitignored at `.gitignore:27`. **`NM-5`, narrowed in R4: R3 wrote "no other temp, backup or sidecar file anywhere", which is broader than any check this Author ran** — it is a statement about the agent's own acts, not a path-level guarantee over directories the harness and OS also use. **The scoped claim stands: this unit created no backup, sidecar or `.orig` file, and issued no command that writes to system temp.** The fresh Reviewer's path-level check found **two GUID `.tmp` files inside the R3 window** — `3da354a9…` at **0 bytes**, timestamped `10:49:09`, matching commit `684c9af` **to the second**, and `9fc35702…` at 41,328 bytes carrying JPEG content. **Re-derived by this Author: both exist as described, among 45 GUID `.tmp` files in that directory spanning 2026-08-14 to 2026-08-22** — a population overwhelmingly predating this unit, which is itself evidence of ambient churn rather than agent writes. **Neither is an agent write and the classification is unaffected** | `AUTHORIZED MUTATION` for the tsbuildinfo · **`ZERO MUTATION` for this unit's own acts** on the named basis · the GUID `.tmp` residue **`HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL`** |
...
**`NEW-1`, added in R5. R4 both wrote and executed, and had no increment of its own** — §17.1's R4 row covered repository files and nothing else, leaving every other `DEC-0040` D13 surface attested only by whole-unit rows written in R1/R2 and last revised in R3, i.e. **claims about a round that had not yet happened.** That is the failure §20.1 names, recreated in §17 by the round that repaired it in §18.7.

| Surface | Finding | Class |
|---|---|---|
| **Repository files** | **Three modified, none created** — this record, `current-state.md`, `context-map.md`. **`apps/web/tsconfig.json` was NOT reopened** (blob `cfdac2ba…`, identical to `f4452a8`), nor was `pnpm-workspace.yaml` (`4ad2e693…`, identical to `df55869`), the lockfile, `.nvmrc`, any manifest or any `packages/orchestrator` file | `AUTHORIZED MUTATION` |
| **Execution performed** | **Read-only, in the primary checkout, no dependency install and no worktree.** `git rev-parse` on two `apps/web/tsconfig.json` blobs; a read of `defaultValueDescription` from `apps/web/node_modules/typescript@5.9.3` **using the pre-existing tree, which was read and not modified, refreshed or used as install evidence**; a directory listing of system temp; `grep -c` on root `package.json` | **`ZERO MUTATION`** — every command was a read |
| **Worktrees, stores, dependency materialization** | **None created.** No `git worktree add`, no `--store-dir`, no `pnpm install`. `git worktree list` showed the three pre-existing entries throughout | `ZERO MUTATION`, on that named basis |
| **`/tmp` and system temp** | **No command R4 issued wrote there.** Its one interaction was the **read-only listing** that re-derived the GUID `.tmp` population. **No `>`, `>>`, `tee`, `-o`, `--output`, `mktemp` or file-landing heredoc was used** | **`ZERO MUTATION` for this round's own acts** · observed residue `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL` |
| **Session scratchpad** | **Unused; re-checked at path level** — `find … -type f` returned `0` | `ZERO MUTATION`, on that named basis |
| **Memory files** | **None written.** No `MEMORY.md`, no project or session memory, no settings file, no `CLAUDE.md` | `ZERO MUTATION`, on that named basis |
| **Preserved breach evidence** | Confirmed present by **directory listing only**, unchanged in size and timestamp. **Not deleted, edited, renamed, moved, recreated or opened** | **`PRESERVED — NOT THIS AUTHOR'S TO DISPOSE OF`** |
| **External mutable systems** | **Zero configuration writes; no network dependency fetch of any kind.** No workflow, ruleset, required check, `CODEOWNERS`, Environment, secret or repository setting; nothing re-armed. `docs/agents/product-orchestrator-agent.md` **not edited** — blob `5e9e6906…`. The only hosted writes were one commit pushed to the existing branch and **PR #95, left open and unmerged** | `ZERO MUTATION` for external configuration, on that named basis |

**No breach is recognized by this Author in R4.**

#### R5 — Author increment

**This round.** It **wrote but did not install, and executed only reads.**

| Surface | Finding | Class |
|---|---|---|
| **Repository files** | **Two modified, none created** — this record and `current-state.md`. **`context-map.md` was NOT reopened in R5**, nor was any file outside those two; the R4 blob identities named above were re-checked and hold | `AUTHORIZED MUTATION` |
| **Execution performed** | **Read-only.** `git rev-parse` / `git diff --numstat` / `git log`, `sed` and `grep` reads of this record for the §17 sweep, a directory listing of system temp, and a `find` count of the session scratchpad. **No dependency install, no worktree, no store, no build, no typecheck** | **`ZERO MUTATION`** — every command was a read |
| **Worktrees, stores, dependency materialization** | **None created**; `git worktree list` shows the three pre-existing entries | `ZERO MUTATION`, on that named basis |
| **`/tmp` and system temp** | **No command R5 issued wrote there**; the one interaction was a read-only listing. **No redirection operator, `tee`, `mktemp` or file-landing heredoc was used at any point** | **`ZERO MUTATION` for this round's own acts** · residue `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL` |
| **Session scratchpad** | **Unused; `find … -type f` returned `0`** | `ZERO MUTATION`, on that named basis |
| **Memory files** | **None written**, of any kind, inside the repository or outside it | `ZERO MUTATION`, on that named basis |
| **Preserved breach evidence** | Confirmed present by **directory listing only**, unchanged. **Untouched** | **`PRESERVED — NOT THIS AUTHOR'S TO DISPOSE OF`** |
| **External mutable systems** | **Zero configuration writes.** One commit pushed to the existing branch; **PR #95 open and unmerged**. `docs/agents/product-orchestrator-agent.md` untouched | `ZERO MUTATION` for external configuration, on that named basis |

**No breach is recognized by this Author in R5.**

#### R6 — Author increment

**This round.** It **wrote but did not install, and executed only reads.** **No technical input changed**, which is verifiable rather than asserted: `apps/web/tsconfig.json` `cfdac2ba…`, `pnpm-workspace.yaml` `4ad2e693…`, `pnpm-lock.yaml` `e183bdc8…`, `packages/orchestrator` tree `7457f3f2…` and `docs/agents/product-orchestrator-agent.md` `5e9e6906…` all hash unchanged from the reviewed `PASS` head.

| Surface | Finding | Class |
|---|---|---|
| **Repository files** | **Two modified, none created** — this record and `context-map.md`. **`current-state.md` was NOT reopened in R6**, and no manifest, lockfile, selector, decision record, agent contract or workflow was touched | `AUTHORIZED MUTATION` |
| **Execution** | **Read-only.** `git rev-parse` / `git status` / `git diff` hash and scope checks, `sed`/`grep`/`awk` reads of this record for the scope sweep, one directory listing of the project memory path, one directory listing of system temp, and one `find` count of the session scratchpad. **No install, package-manager probe, worktree, store, build or typecheck** | **`ZERO MUTATION`** — every command was a read |
| **Worktrees, stores, dependency materialization** | **None created**; `git worktree list` shows the three pre-existing entries | `ZERO MUTATION`, on that named basis |
| **`/tmp` and system temp** | **No command R6 issued wrote there**; the one interaction was a read-only listing. **No `>`, `>>`, `tee`, `-o`, `--output`, `mktemp` or file-landing heredoc was used at any point** | **`ZERO MUTATION` for this Author's acts** · ambient residue `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL` |
| **Session scratchpad** | **Unused; `find … -type f` returned `0`**, this Author's own session only | `ZERO MUTATION`, on that named basis |
| **Memory files** | **None written.** Re-verified unit-wide at R6 on the shared project memory path: newest mtime **`2026-08-21 12:52:34`**, predating the unit window | `ZERO MUTATION`, on that named basis |
| **Preserved breach evidence** | Confirmed present by **directory listing only**, unchanged in size and timestamp. **Not deleted, edited, renamed, moved, recreated or opened** | **`PRESERVED — NOT THIS AUTHOR'S TO DISPOSE OF`** |
| **External mutable systems** | **Zero configuration writes.** One commit pushed to the existing branch; **PR #95 open and unmerged**. No workflow, ruleset, required check, `CODEOWNERS`, Environment, secret or repository setting; nothing re-armed | `ZERO MUTATION` for external configuration, on that named basis |

**No breach is recognized by this Author in R6.**

#### R7 — Author increment

**This round. Documentation-only repository mutation and read-only verification.** **No technical input changed**, verifiable rather than asserted: `apps/web/tsconfig.json` `cfdac2ba…`, `pnpm-workspace.yaml` `4ad2e693…`, `pnpm-lock.yaml` `e183bdc8…`, `.nvmrc` `209e3ef4…`, root `package.json` `b00dd1ca…`, `apps/web/package.json` `bd778718…`, `packages/shared/package.json` `3334d2cf…`, `packages/orchestrator` tree `7457f3f2…`, `docs/agents/product-orchestrator-agent.md` `5e9e6906…` and `current-state.md` `f8bd0a7a…` all hash unchanged from `9e93161`.

| Surface | Finding | Class |
|---|---|---|
| **Repository files** | **One modified, none created — this record alone.** **`current-state.md` was NOT reopened, and `context-map.md` remained unchanged**, no contradiction having been found that required it | `AUTHORIZED MUTATION` |
| **Execution** | **Read-only.** `git rev-parse` hash verification of every declared invariant, `git status` / `git diff --numstat`, and `sed`/`grep` reads of this record for the rule-4 pass. **No install, package-manager probe, typecheck, build, worktree or store** | **`ZERO MUTATION`** — every command was a read |
| **Worktrees, stores, dependency materialization** | **None created**; `git worktree list` shows the three pre-existing entries | `ZERO MUTATION`, on that named basis |
| **`/tmp` and system temp** | **No command R7 issued wrote there.** **No `>`, `>>`, `tee`, `-o`, `--output`, `mktemp` or file-landing heredoc was used at any point** | **`ZERO MUTATION` for this Author's acts** · ambient residue `HARNESS / SYSTEM PERSISTENCE OUTSIDE AGENT CONTROL` |
| **Session scratchpad** | **Unused**, this Author's own session only | `ZERO MUTATION`, on that named basis |
| **Memory files** | **None written**, inside the repository or outside it | `ZERO MUTATION`, on that named basis |
| **Preserved breach evidence** | **Not touched, and not listed** — R7 needed no path-level temp check, so it made none | **`PRESERVED — NOT THIS AUTHOR'S TO DISPOSE OF`** |
| **External mutable systems** | **Zero configuration writes.** One commit pushed to the existing branch and read-only `gh pr view`; **PR #95 open and unmerged** | `ZERO MUTATION` for external configuration, on that named basis |

**No breach is recognized by this Author in R7.**
...
### 18.1 Independence — the mandated label, and the limitation it carries

Each reviewer held **all three required review functions** — independent technical/toolchain, independent reproducibility, and independent governance/scope — under `DEC-0037` D5 and D8, and each is **distinct from this Author**.

> **Independence type, stated in the exact label `DEC-0040` D10 item 4 permits:**
>
> **`PROCESS-INDEPENDENT · PRINCIPAL-NOT-INDEPENDENT`**
>
> **Both halves are load-bearing and neither may be dropped**, which is what that clause says in terms. It applies to **every** reviewer of this unit: the R1/R2 Reviewer, and the **fresh** reviewer that returned the R3 `HOLD` and this R4 round.

**What the second half means concretely, recorded at full strength rather than smoothed:** every review ran on the **same machine** and under the **same GitHub credential** as this Author, so all parties act as **Principal A**. `DEC-0037` D7.7 and D12, and `DEC-0039` D6, forbid representing that arrangement as principal independence, and no reviewer claimed it. **`DEC-0040` D10 item 5 additionally forbids inferring principal separation from instance separation** — two distinct agent instances are two instances, not two principals — and nothing in this section should be read as supplying what D10.4's second half withholds.

> **`NM-6`, repaired in R4.** R2 and R3 wrote this as *"`TECHNICAL PRINCIPAL: UNKNOWN / NOT INDEPENDENT`"* and *"function-independent … not principal-independent"*. **Both halves of the substance were conveyed and no reader was misled**, but **neither string is the label D10 item 4 permits**, and that clause admits no paraphrase. It also conflated two distinct fields: the D10 item 3 *principal* entry and the D10 item 4 *independence-type* label. **The mandated label is now used, here and in the header.**
...
|---|---|---|
| **T1 — ALIGN** | **SELECTED** | The config was the untrue half. Inheritance is what every sibling does, what the base exists for, and what two independent documents describe. The change is **one line**, adds strictness and removes nothing, and **makes the existing README statement true without editing it**. **Behaviourally it adds four options, not five** — `forceConsistentCasingInFileNames` already defaults to `true` (§19.4, `NM-2`) — and **`target` is deliberately left at `ES2017`**, so the `D0` §10.2 row 1 defect is closed **in part, not in whole** (§19.6) |
...
> **`NM-2`, repaired in R4 — five keys, but FOUR behavioural changes.** **`forceConsistentCasingInFileNames` already defaults to `true` in TypeScript 5.9.3**, so inheriting it changes what the configuration *reports*, not what the compiler *does*. Re-derived by this Author rather than taken on report: the option's declaration in the installed `typescript@5.9.3` carries **`defaultValueDescription: true`**. The literal table label above — *"compilerOptions ADDED by extends"* — **is accurate and stands**; it is the **prose** elsewhere that read as five behavioural gains, and it is corrected at §19.3 and in both routing documents. *(`NEW-5`, repaired in R5: R4 also named §21.2 here. The Reviewer diffed the §21 region across both revisions and found it **byte-identical**, carrying no option-count claim at all — so no correction was made there and none was needed. The false cross-reference is struck rather than satisfied by editing §21, which is closed.)*

> **`NM-3`, added in R4 — what this green typecheck does NOT prove, bounded here as §11 already bounds the R1/R2 run.** The `apps/web` program is **four files and roughly fifty-seven lines**. **Nothing in it indexes an array, nothing declares a class, and `noEmit: true` suppresses every emit path** — so of the four behavioural options, **`noUncheckedIndexedAccess`, `noImplicitOverride` and `declaration` are exercised by no code in the program.** `verbatimModuleSyntax` is genuinely exercised, by `import type { Metadata }` and the side-effect `import "./globals.css"`. **The exit 0 therefore establishes that alignment breaks nothing today; it does NOT establish that the inherited strictness has been tested against code that could violate it.** Stating otherwise would be exactly the `DEC-0042` §D10 defect of a signal claiming more than its mechanism observes.
>
> **The fresh Reviewer supplied the negative control this Author did not, and it strengthens T1 beyond what was established here.** With the `extends` in place, a deliberate probe produced **`error TS2322: Type 'string | undefined' is not assignable to type 'string'` (exit 2)**; with the `extends` removed, the identical probe **exited 0**. **`noUncheckedIndexedAccess` is therefore live rather than cosmetic** — the strictness is real and merely unexercised by the current four files. **Recorded as the Reviewer's result, not as this Author's.**
...
| Inheriting the base's `exclude`, which lists **`.next`**, would drop `.next/types/**/*.ts` because **`exclude` beats `include`** | **DID NOT OCCUR** | the child declares its own `exclude`, replacing the parent's entirely. **The probative observable is the RESOLVED value: `tsc --showConfig` reports `"exclude": ["node_modules"]` in the after-state, so the base's `.next` entry did not survive the merge** |

> **`NM-1`, repaired in R4 — the grounds were wrong even though the conclusion was right, and that distinction is worth more than the conclusion.** R3 wrote that this hazard was *"proven by the file set being identical"*. **It was not.** The validation worktree contained **no `.next` directory at all**, so **no `.next` file could enter the program under either configuration** and the 4→4 count is equally consistent with the hazard having occurred. **A measurement that cannot distinguish the two outcomes proves neither.** The resolved `exclude` above is the observable that actually discriminates, and it was captured in the same `--showConfig` run — the evidence was in hand and the wrong half of it was cited.
>
> **The fresh Reviewer then ran the test this Author did not:** with `.next/types` **present**, the program resolves to **9 files and `tsc --noEmit` still exits 0** — the hazard directly falsified rather than merely argued away. **Recorded as the Reviewer's result, not as this Author's.** Its stated limitation is carried with it: **that `.next` tree was generated under the pre-R3 configuration, so the behaviour of a `.next` regenerated under the aligned config is `UNPROVEN`.**
...
- **That the `D0` §10.2 row 1 defect is closed in whole** — it is **not**. That row's stated consequence includes *"and targets an older ECMAScript level"*, and **T1 deliberately leaves `target` at `ES2017`**, as a Next.js app legitimately may. **The inheritance half is closed; the `target` half is untouched and not withdrawn.** `NM-4`, narrowed in R4 here and in both routing documents.
- **That the inherited strictness has been exercised** — see the `NM-3` bound at §19.4. **Live, per the Reviewer's negative control; unexercised by the four files in the program.**

**Two forward observations from the fresh Reviewer — RECORDED AND ROUTED, deliberately NOT repaired in R4.**

1. **`declaration: true` inherited alongside `allowJs: true` on `apps/web`.** `packages/orchestrator/tsconfig.json` explicitly overrides `declaration: false` under the same base; **`apps/web` does not.** **Inert today** — `noEmit: true` is set and the typecheck exits 0, measured — **but if a future unit removes `noEmit` or runs an emitting `tsc`, that pairing needs re-testing.** `UNPROVEN` for any emitting configuration. → **routed to whichever later unit first makes `apps/web` emit; not a `D1` question and not repaired here.**
2. **`docs/foundation/monorepo-structure.md:28` is inaccurate about root `package.json`.** It describes that file as declaring *"workspaces (apps/*, packages/*)"*; **root `package.json` carries no `workspaces` key at all** — pnpm reads `pnpm-workspace.yaml` — so the claim was already wrong before this unit, and **`D1`'s landing additionally makes its glob description stale**, since the active declaration now carries the `!packages/orchestrator` negation. **Disclosed against this Author's own reasoning:** §19.2 item 3 cites **line 30 of that same file** as affirmative evidence for T1, and R3 did not check line 28 against its own landing. **The T1 conclusion does not depend on line 28**, which describes a different file, so the evidence at item 3 stands — **but citing a document as evidence without noticing an adjacent defect it contains is a real gap, and it is recorded rather than quietly fixed.** **`docs/foundation/monorepo-structure.md` is outside R4's write scope** → **routed; not repaired.**
