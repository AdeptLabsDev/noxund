...
>
> **The three failures, named rather than buried:** R2 wrote six edits where it had made seven; R3 left a four-file blob-identity list after landing a fifth file (`M-2`); R4 repaired that row but left *"the four files above"* in the row beneath it (`NEW-2`), left *"the four in the write scope above"* in §17.1 (`NEW-A`, found by R5's own sweep rather than reported to it), and wrote *"seven"* over an enumeration of eight (`NEW-4`). **Each round repaired only the count it was told about. R5 swept the whole section instead.**
...
**Documentation-only, and included because the rule above admits no exception for a round that merely writes.**
...
| **`/tmp`, system temp, scratchpad, memory, worktrees, stores** | **Nothing created on any of them.** No redirection operator, `tee` or `mktemp`; the pull-request body had been passed inline in R1 and every commit message via stdin | `ZERO MUTATION`, on that named basis |
