...
| 8 | **No checker for two further active languages.** No shellcheck over the shell scripts, no actionlint over the workflows, and no configuration validation. **No SQL linter is present either — that current fact stands and is not withdrawn.** Its Phase-D reading is what changes: the only SQL corpus this unit identified is `supabase/**`, classified `HISTORICAL / PRESERVED` at §2, so that absence is **not by itself a Phase-D quality gap** and requires no `D4` investment (`DEC-0042` §D11). The count in this heading is re-derived on that basis rather than left stale | `git grep -rn -i 'shellcheck\|actionlint\|sqlfluff\|ruff' -- .github/` returns no match | `VERIFIED` |
...
| **Q3** | Automated test surface | **PARTIAL** · `VERIFIED` | Strong on Python — 276 unit tests plus 40 governance tests, stdlib-only and deterministic. **The active JS/TS surfaces have zero test assets** — `apps/web` and `packages/shared`, established by search rather than by an absent devDependency (§5). **`packages/orchestrator` does have a suite**, 36 tests, but it is `LEGACY / NON-AUTHORITATIVE`, it runs in no CI, and it receives **no Phase-D quality investment** (`DEC-0042` §D11). Stated in the vocabulary `DEC-0042` §D7 requires: on the active surfaces `TESTS EXIST` is false; on the legacy one `TESTS EXIST` is true while `TESTS COVER CURRENT ACTIVE BEHAVIOR` and `TESTS RUN IN CI` are both false | Every active surface either has a suite exercising its relied-on behaviour, or a recorded decision that it needs none | **route** — `D2`, under its **JS/TS test-surface adjudication** duty. **Not `D0`'s to design**, and not `D5`'s to decide |
...
**Active surface.** Root JS/TS workspace configuration, `apps/web`, `packages/shared`. **`packages/orchestrator` is not an active `D1` target** — it is `LEGACY / NON-AUTHORITATIVE` (§2) — and is named anywhere in this entry only because the root configuration refers to it. **Prerequisite.** None.
**The boundary, stated so `D1` is read neither too narrowly nor too widely.** The root and workspace findings are real and survive in full: root configuration declares `packages/*`, **which includes the legacy package**; the lockfile omits that declared workspace; and the root Node constraint does not satisfy that package's declared requirement (§9). These are legitimate root/workspace-coherence observations and not one of them is withdrawn. **`D1` may reconcile active and root workspace configuration, including deciding how the legacy package is represented by that root configuration** — but it may **not** edit any file under `packages/orchestrator/**`, make its tests runnable, modernize its dependencies, or otherwise invest engineering-quality effort in that legacy surface (`DEC-0042` §D11; `DEC-0038` D3). **No workspace change occurs in `D0`.**
...
**Must NOT absorb.** Adding CI; adopting any test framework or linter; dependency upgrades beyond what coherence requires; **any mutation under `packages/orchestrator/**`, and any quality investment in that legacy surface.**
...
**Second named responsibility — the `Q3` test-surface adjudication.** `Q3` is routed here and nowhere else, because `D5` is documentation closeout and cannot be the first venue that decides whether active product behaviour needs regression tests. **For each active JS/TS surface — `apps/web` and `packages/shared` — `D2` must eventually determine either that a behavioural test suite is justified now, or that none is currently justified, with an explicit evidence-backed reason.** This is an **adjudication requirement, not an automatic framework-adoption decision**, and `packages/orchestrator` falls outside it (`DEC-0042` §D11).
**Expected artifact.** One workflow, plus the recorded `Q3` adjudication above. **Mutating:** YES.
**Must NOT absorb.** Any ruleset, branch-protection or required-status-check change — an authority interaction under `DEC-0041` D5 and D10; any Python surface; and **framework adoption by default**. `D2` must adjudicate `Q3`, but **a framework must not be adopted merely because `Q3` exists**, and **`D0` pre-authorizes none** — not Jest, not Vitest, not Playwright, not any other. Should the adjudication conclude that a new test dependency is justified, **implementing it requires explicit scope in the future Product-Lead `D2` GO, or a separately authorized bounded follow-up unit** (`DEC-0042` §D6, §D16).
...
**Exact problem.** Ruff is configured and suppressed against but installed and run nowhere, over the largest active code body in the repository. The shell scripts and the workflows have no checker at all. **SQL is deliberately not among `D4`'s targets.** No SQL linter is present — that fact is recorded at §10.2 row 8 — but the only SQL corpus in the tree is `supabase/**`, which §2 classifies `HISTORICAL / PRESERVED` and which `DEC-0042` §D11 places outside all quality investment in terms, *"no linter pointed at them"*. The absence is therefore a fact this baseline states, not a gap this roadmap routes.
**Active surface.** `services/data-engine`, `tools/governance`, `infra/postgres`, `.github/workflows` — the genuinely active non-TypeScript engineering surfaces this baseline justifies, **each still subject to `D4`'s own per-surface adjudication rather than assumed to warrant a tool**. **Prerequisite: `D2`** — a place to run it.
...
**Must NOT absorb.** Thresholds; **any Phase-D investment in `supabase/**` whatsoever** — it is neither a lint target nor a content-edit target for this unit, because `DEC-0042` §D11 gives a historical, preserved surface no quality investment and equally forbids editing, emptying or moving it (`DEC-0038` D3).
...
**Later rounds, same unit and same base.** This record was produced in R1 and revised in the correction rounds disclosed in the table below, every one of them under the same Product-Lead GO. **No round count is written into this sentence, for the reason §18 gives:** a further round would falsify one. **The canonical base never moved** — every round was authored, verified and pushed against `main` @ `9f12680d329a6107693bdd9f1b077fdfb17dd0dc` — and **no round touched a file outside the four in the write scope above.**
...
| **R4** | Corrected the mutation-accounting row above, which stated a push count the later rounds had already falsified, and added this disclosure |
| **R5** | This round. Six bounded corrections confined to §10.2, §11 and §16. `Q3` now separates the **active** JS/TS surfaces, which have zero test assets, from the legacy package, which has a suite that runs nowhere and earns no investment. `D4` no longer names historical `supabase` SQL as a lint target. §10.2 row 8 and §15 item 8 keep the fact that no SQL linter is present while withdrawing the claim that its absence is an active Phase-D gap. `D1`'s active surface excludes the legacy package while every root and workspace finding survives intact. `Q3` gains an owner in `D2`, and `D2` is barred from pre-authorizing any test framework. **No `D` unit was added, the `D1` → `D5` sequence is unchanged, `D1` remains the `NEXT CANDIDATE`, and the CI-history findings at §6.1, §15 item 1 and `D2`'s stated problem are untouched** |
